...
@property (nonatomic, strong) NSMutableDictionary<NSNumber *, ZegoPictureCapturer *> *pictureCapturerMap;

@property (nonatomic, strong) NSMutableDictionary<NSNumber *, ZegoCustomAudioSource *> *customAudioSourceMap;
...
@property (nonatomic, strong) NSSet *logSkillMethods;
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    if (![self.logSkillMethods containsObject:call.method]) {
...
- (void)loginRoom:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (configMap && configMap.count > 0) {
        ...
        if (configMap[@"roomType"]) {
            configObject.roomType = [ZegoUtils unsignedIntValue:configMap[@"roomType"]];
        }
    }
...
- (void)startPublishingStream:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (configMap && configMap.count > 0) {
        ...
        config.streamCensorFlag = [ZegoUtils intValue:configMap[@"streamCensorFlag"]];
...
- (void)startPreview:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (canvasMap && canvasMap.count > 0) {
        ...
        int rotation = [ZegoUtils intValue:canvasMap[@"rotation"]];
...
- (void)takePublishStreamSnapshotByConfig:(FlutterMethodCall *)call result:(FlutterResult)result {
    int channel = [ZegoUtils intValue:call.arguments[@"channel"]];
    NSDictionary *configMap = call.arguments[@"config"];
    ZegoPublisherTakeSnapshotConfig *config = [ZegoPublisherTakeSnapshotConfig new];
    config.position = (ZegoPublisherTakeSnapshotPosition)[ZegoUtils intValue:configMap[@"position"]];

    [[ZegoExpressEngine sharedEngine] takePublishStreamSnapshotByConfig:config callback:^(int errorCode, ZGImage * _Nullable image) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            NSData *imageData = nil;
            if (errorCode == 0 && image) {
#if TARGET_OS_IPHONE
...
- (void)getVideoEncoderSupported:(FlutterMethodCall *)call result:(FlutterResult)result {
    ZegoVideoCodecID codecID = (ZegoVideoCodecID)[ZegoUtils intValue:call.arguments[@"codecID"]];
    ZegoVideoCodecBackend codecBackend = (ZegoVideoCodecBackend)[ZegoUtils intValue:call.arguments[@"codecBackend"]];

    [[ZegoExpressEngine sharedEngine] getVideoEncoderSupported:codecID codecBackend:codecBackend callback:^(int support) {
...
- (void)setAudioSource:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (![ZegoUtils isNullObject:call.arguments[@"config"]]) {
        ...
        NSArray<NSNumber *> *audioSourceIndexList = configMap[@"audioSourceIndexList"];
        config.audioSourceCount = (int)audioSourceIndexList.count;
        int *audioSources = NULL;
        if (config.audioSourceCount > 0) {
            audioSources = malloc(sizeof(int)*config.audioSourceCount);
        }
        config.audioSourceIndexList = audioSources;
        for(NSNumber * index in audioSourceIndexList) {
            *audioSources = [index intValue];
            audioSources += 1;
        }

        config.enableMixScreenCapture = [ZegoUtils boolValue:configMap[@"enableMixScreenCapture"]];
    }
    ...
    if (!hasChannel && !hasConfig) {
        ...
        free(config.audioSourceIndexList);
    } else {
        if (channel == 0) {
            ret = [[ZegoExpressEngine sharedEngine] setAudioSource:(ZegoAudioSourceType)source config:config];
        }
...
- (void)createCustomAudioSource:(FlutterMethodCall *)call result:(FlutterResult)result {

    if (!self.customAudioSourceMap) {
        self.customAudioSourceMap = [NSMutableDictionary dictionary];
    }

    ZegoCustomAudioSource *audioSource = [[ZegoExpressEngine sharedEngine] createCustomAudioSource];

    if (audioSource) {
        int index = [audioSource getIndex];
        self.customAudioSourceMap[@(index)] = audioSource;
        result(@(index));
    } else {
        result(@(-1));
    }
...
- (void)destroyCustomAudioSource:(FlutterMethodCall *)call result:(FlutterResult)result {

    NSNumber *index = call.arguments[@"index"];
    ZegoCustomAudioSource *audioSource = self.customAudioSourceMap[index];

    if (audioSource) {
        [[ZegoExpressEngine sharedEngine] destroyCustomAudioSource:audioSource];
        [self.customAudioSourceMap removeObjectForKey:index];
    }

    result(nil);
}

- (void)customAudioSourceStartCapture:(FlutterMethodCall *)call result:(FlutterResult)result {

    NSNumber *index = call.arguments[@"index"];
    ZegoCustomAudioSource *audioSource = self.customAudioSourceMap[index];

    if (audioSource) {
        result(@([audioSource startCapture]));
    } else {
        result(@(-1));
    }
}

- (void)customAudioSourceStopCapture:(FlutterMethodCall *)call result:(FlutterResult)result {

    NSNumber *index = call.arguments[@"index"];
    ZegoCustomAudioSource *audioSource = self.customAudioSourceMap[index];

    if (audioSource) {
        result(@([audioSource stopCapture]));
    } else {
        result(@(-1));
    }
}

- (void)customAudioSourceEnableAEC:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (audioSource) {
        result(@([audioSource enableAEC:enable]));
    } else {
        result(@(-1));
    }
}

- (void)customAudioSourceSetVolume:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (audioSource) {
        result(@([audioSource setVolume:volume]));
    } else {
        result(@(-1));
    }
}

- (void)customAudioSourceMute:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (audioSource) {
        result(@([audioSource mute:mute]));
    } else {
        result(@(-1));
    }
}

- (void)customAudioSourcePushAudioFrame:(FlutterMethodCall *)call result:(FlutterResult)result {

    NSNumber *index = call.arguments[@"index"];
    FlutterStandardTypedData *data = call.arguments[@"data"];
    int rawDataLength = [ZegoUtils intValue:call.arguments[@"dataLength"]];
    if (!data || rawDataLength <= 0) {
        result(@(-1));
        return;
    }
    unsigned int dataLength = (unsigned int)rawDataLength;
    ...
    if (audioSource) {
        unsigned int safeDataLength = MIN(dataLength, (unsigned int)data.data.length);
        result(@([audioSource pushAudioFrame:(unsigned char *)[data.data bytes]
                                 dataLength:safeDataLength
                                      param:param]));
    } else {
        result(@(-1));
    }
...
- (void)enableVideoEncoderEnhancement:(FlutterMethodCall *)call result:(FlutterResult)result {

    BOOL enable = [ZegoUtils boolValue: call.arguments[@"enable"]];
    int channel = [ZegoUtils intValue:call.arguments[@"channel"]];
    float enhanceLevel = [ZegoUtils floatValue:call.arguments[@"enhanceLevel"]];
    
    [[ZegoExpressEngine sharedEngine] enableVideoEncoderEnhancement:enable enhanceLevel:enhanceLevel channel:(ZegoPublishChannel)channel];

    result(nil);
}

- (void)enableVideoEncoderEnhancementWithConfig:(FlutterMethodCall *)call result:(FlutterResult)result {

    BOOL enable = [ZegoUtils boolValue: call.arguments[@"enable"]];
    int channel = [ZegoUtils intValue:call.arguments[@"channel"]];

    ZegoVideoEncoderEnhancementConfig *config = [[ZegoVideoEncoderEnhancementConfig alloc] init];
    NSDictionary *configMap = call.arguments[@"config"];
    if (configMap) {
        config.enhanceLevel = [ZegoUtils floatValue:configMap[@"enhanceLevel"]];
        config.enhanceLevelMode = (ZegoVideoEncodeEnhanceLevelMode)[ZegoUtils intValue:configMap[@"enhanceLevelMode"]];
    }
        
    [[ZegoExpressEngine sharedEngine] enableVideoEncoderEnhancementWithConfig:enable config:config channel:(ZegoPublishChannel)channel];
...
- (void)startPlayingStream:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (canvasMap && canvasMap.count > 0) {
        ...
        BOOL alphaBlend = [ZegoUtils boolValue:canvasMap[@"alphaBlend"]];
        int rotation = [ZegoUtils intValue:canvasMap[@"rotation"]];
        BOOL mirror = [ZegoUtils boolValue:canvasMap[@"mirror"]];
...
- (void)switchPlayingStream:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (playerConfigMap && playerConfigMap.count > 0) {
        ...
        NSDictionary * switchStreamConfigMap = playerConfigMap[@"switchStreamConfig"];
        if (switchStreamConfigMap && switchStreamConfigMap.count > 0) {
            ZegoSwitchPlayingStreamConfig *switchStreamConfig = [[ZegoSwitchPlayingStreamConfig alloc] init];
            switchStreamConfig.switchType = (ZegoSwitchPlayingStreamType)[ZegoUtils intValue:switchStreamConfigMap[@"switchType"]];
            switchStreamConfig.switchTimeout = [ZegoUtils intValue:switchStreamConfigMap[@"switchTimeout"]];
...
- (void)switchPlayingResource:(FlutterMethodCall *)call result:(FlutterResult)result {

    NSString *streamID = call.arguments[@"streamID"];
    ZegoResourceType resourceType =
        (ZegoResourceType)[ZegoUtils intValue:call.arguments[@"resourceType"]];

    [[ZegoExpressEngine sharedEngine] switchPlayingResource:streamID resourceType:resourceType];
...
- (void)getVideoDecoderSupported:(FlutterMethodCall *)call result:(FlutterResult)result {
    ZegoVideoCodecID codecID = (ZegoVideoCodecID)[ZegoUtils intValue:call.arguments[@"codecID"]];
    ZegoVideoCodecBackend codecBackend = (ZegoVideoCodecBackend)[ZegoUtils intValue:call.arguments[@"codecBackend"]];

    [[ZegoExpressEngine sharedEngine] getVideoDecoderSupported:codecID codecBackend:codecBackend callback:^(int support) {
        result(@{@"support": @(support)});
    }];
}
...
- (void)setPlayingCanvas:(FlutterMethodCall *)call result:(FlutterResult)result {
#if TARGET_OS_IPHONE
    NSString *streamID = call.arguments[@"streamID"];
    ZegoViewUpdateType updateType = (ZegoViewUpdateType)[ZegoUtils intValue:call.arguments[@"updateType"]];
    ...
    if (canvasMap && canvasMap.count > 0) {
        ...
        int rotation = [ZegoUtils intValue:canvasMap[@"rotation"]];
        BOOL mirror = [ZegoUtils boolValue:canvasMap[@"mirror"]];
        NSString *viewContext = canvasMap[@"viewContext"];
        ...
        if (self.enablePlatformView) {
            ...
            if (platformView) {
                ...
                errorCode = [[ZegoExpressEngine sharedEngine] setPlayingCanvas:streamID updateType:updateType canvas:canvas];
            } else {
                // Play video without creating the PlatformView in advance
                // Need to invoke dart `createPlatformView` method in advance to create PlatformView and get viewID (PlatformViewID)
                NSString *errorMessage = [NSString stringWithFormat:@"The PlatformView for viewID:%ld cannot be found, developer should call `createPlatformView` first and get the viewID", (long)viewID];
                ZGError(@"[setPlayingCanvas] %@", errorMessage);
                result([FlutterError errorWithCode:[@"setPlayingCanvas_No_PlatformView" uppercaseString] message:errorMessage details:nil]);
                return;
            }

        } else {
            NSString *errorMessage = [NSString stringWithFormat:@"setPlayingCanvas Supported only PlatformView"];
            ZGError(@"[setPlayingCanvas] %@", errorMessage);
            result([FlutterError errorWithCode:[@"setPlayingCanvas_Supported_Only_PlatformView" uppercaseString] message:errorMessage details:nil]);
            return;
        }
    ...
    result(@(errorCode));
#elif TARGET_OS_OSX
    NSString *errorMessage = [NSString stringWithFormat:@"OSX platform not support feature"];
    ZGError(@"[setPlayingCanvas] %@", errorMessage);
    result([FlutterError errorWithCode:[@"not_support_feature" uppercaseString] message:errorMessage details:nil]);
#endif
}
...
- (void)startMixerTask:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (inputListMap && inputListMap.count > 0) {
        ...
        for (NSDictionary *inputMap in inputListMap) {
            ...
            NSDictionary *blurInfoMap = inputMap[@"blurInfo"];
            if(blurInfoMap && ![ZegoUtils isNullObject:blurInfoMap]) {
                ZegoBlurInfo *blurInfo = [[ZegoBlurInfo alloc] init];
                blurInfo.topPadding = [ZegoUtils intValue:blurInfoMap[@"topPadding"]];
                blurInfo.leftPadding = [ZegoUtils intValue:blurInfoMap[@"leftPadding"]];
                blurInfo.bottomPadding = [ZegoUtils intValue:blurInfoMap[@"bottomPadding"]];
                blurInfo.rightPadding = [ZegoUtils intValue:blurInfoMap[@"rightPadding"]];
...
- (void)startAutoMixerTask:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    taskObject.streamAlignmentVolumeControlMode = (ZegoStreamAlignmentVolumeControlMode)[ZegoUtils intValue:call.arguments[@"streamAlignmentVolumeControlMode"]];

    // streamAlignmentBaselineStreamID
    taskObject.streamAlignmentBaselineStreamID = call.arguments[@"streamAlignmentBaselineStreamID"];
...
- (void)enableAudioCaptureDevice:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    [[ZegoExpressEngine sharedEngine] enableAudioCaptureDeviceAsync:enable callback: ^(int errorCode) {
        result(nil);
    }];
}
...
- (void)enableBeforeAudioPrepAudioData:(FlutterMethodCall *)call result:(FlutterResult)result {
    BOOL enable = [ZegoUtils boolValue:call.arguments[@"enable"]];
    NSDictionary *paramMap = call.arguments[@"param"];
    ...
    [[ZegoExpressEngine sharedEngine] enableBeforeAudioPrepAudioData:enable param:param];
...
- (void)sendCustomAudioCaptureOpusData:(FlutterMethodCall *)call result:(FlutterResult)result {

    FlutterStandardTypedData *data = call.arguments[@"data"];
    int dataLength = [ZegoUtils intValue:call.arguments[@"dataLength"]];
    unsigned long long referenceTimeMillisecond = [ZegoUtils unsignedLongLongValue:call.arguments[@"referenceTimeMillisecond"]];
    int samples = [ZegoUtils intValue:call.arguments[@"samples"]];
    NSDictionary *paramMap = call.arguments[@"param"];
    ...
    [[ZegoExpressEngine sharedEngine] sendCustomAudioCaptureOpusData:(unsigned char*)[data.data bytes] dataLength:dataLength timestamp:timestamp samples:samples param:param channel:channel];
...
- (void)enableCustomVideoRender:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (!self.enablePlatformView) {
        if (enable) {
            [[ZegoExpressEngine sharedEngine] enableCustomVideoRender:true config:config];
...
- (void)enableAudioCaptureWithConfigScreenCaptureSource:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (screenCaptureSource) {
        ...
        config.enableWindowCapture = [ZegoUtils boolValue:configMap[@"enableWindowCapture"]];
    
        [screenCaptureSource enableAudioCapture:enable config:config];
    }
...
- (void)updateScreenCaptureConfigScreenCaptureSource:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    config.orientation = (ZegoScreenCaptureOrientation)[ZegoUtils intValue:configMap[@"orientation"]];
    config.muteExtensMicrophone = [ZegoUtils boolValue:configMap[@"muteExtensMicrophone"]];
    config.audioDeviceMode = (ZegoScreenCaptureAudioDeviceMode)[ZegoUtils intValue:configMap[@"audioDeviceMode"]];
...
#pragma mark - Picture Capturer

- (void)createPictureCapturer:(FlutterMethodCall *)call result:(FlutterResult)result {

    if (!self.pictureCapturerMap) {
        self.pictureCapturerMap = [NSMutableDictionary dictionary];
    }

    ZegoPictureCapturer *pictureCapturer = [[ZegoExpressEngine sharedEngine] createPictureCapturer];

    if (pictureCapturer) {
        int index = pictureCapturer.getIndex;

        self.pictureCapturerMap[@(index)] = pictureCapturer;
...
- (void)destroyPictureCapturer:(FlutterMethodCall *)call result:(FlutterResult)result {

    NSNumber *index = call.arguments[@"index"];
    ZegoPictureCapturer *pictureCapturer = self.pictureCapturerMap[index];

    if (pictureCapturer) {
        [[ZegoExpressEngine sharedEngine] destroyPictureCapturer:pictureCapturer];
    }

    [self.pictureCapturerMap removeObjectForKey:index];

    result(nil);
}

- (void)setPathPictureCapturer:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (pictureCapturer) {
        ...
        NSString *flutterAssetPrefix = @"flutter-asset://";
        if ([path hasPrefix:flutterAssetPrefix]) {
            NSString *assetName = [path substringFromIndex:flutterAssetPrefix.length];
#if TARGET_OS_IPHONE
...
#endif
            NSString *processedURL = [NSString stringWithFormat:@"file:%@", assetRealPath];
            [pictureCapturer setPath:processedURL];
        } else {
...
- (NSSet *)logSkillMethods {
    if (!_logSkillMethods) {
        _logSkillMethods = [NSSet setWithObjects:
                            @"customAudioSourcePushAudioFrame",
                            nil];
    }
    return _logSkillMethods;
}
