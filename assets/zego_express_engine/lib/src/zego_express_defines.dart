...
enum ZegoScreenCaptureAudioDeviceMode {
  /// Default mode.
  None,
...
enum ZegoVideoEncodeEnhanceLevelMode {
...
class ZegoRoomConfig {
  ...
  ZegoRoomConfig(this.maxMemberCount, this.isUserStatusNotify, this.token,
      {this.capabilityNegotiationTypes, this.roomType});
...
///
/// Extended parameters for the [switchPlayingStream] interface.
class ZegoSwitchPlayingStreamConfig {
  /// Switch playing stream type.
  ...
  ZegoSwitchPlayingStreamConfig(this.switchType, this.switchTimeout);
}
...
class ZegoBlurInfo {
  ...
  ZegoBlurInfo(
      this.topPadding, this.leftPadding, this.bottomPadding, this.rightPadding);
...
class ZegoMixerInput {
  ...
  ZegoMixerInput.defaultConfig()
      : this.streamID = "",
        ...
        this.cornerRadius = 0,
        this.blurInfo = ZegoBlurInfo(0, 0, 0, 0);
}
...
class ZegoAutoMixerTask {
  ...
  String streamAlignmentBaselineStreamID;
  ...
  ZegoAutoMixerTask()
      : taskID = "",
        ...
        streamAlignmentVolumeControlMode =
            ZegoStreamAlignmentVolumeControlMode.Close,
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'minPlayStreamBufferLength': this.minPlayStreamBufferLength,
      'streamAlignmentVolumeControlMode':
          this.streamAlignmentVolumeControlMode.index,
      'streamAlignmentBaselineStreamID': this.streamAlignmentBaselineStreamID
    };
  }
}
...
/// File recording progress.
class ZegoDataRemoteRecordProgress {
  /// Current recording duration in milliseconds
  int duration;
  ...
  ZegoDataRemoteRecordProgress(
      this.duration, this.currentFileSize, this.quality);
}
...
class ZegoScreenCaptureAudioConfig {
  ...
  ZegoScreenCaptureAudioConfig(this.enableWindowCapture,
      {this.excludeProcessID});
}
...
class ZegoAudioSourceMixConfig {
  ...
  ZegoAudioSourceMixConfig(
      ...
      this.enableMixScreenCapture,
      this.audioSourceIndexList});
}
...
class ZegoAudioSourceProcessConfig {
  /// Audio capture source mix instance index list.
  List<int> mixAudioSourceIndexList;

  ZegoAudioSourceProcessConfig(this.mixAudioSourceIndexList);
}
...
class ZegoVideoEncoderEnhancementConfig {
  /// The mode of object enhance level mode.
  ZegoVideoEncodeEnhanceLevelMode enhanceLevelMode;

  /// enhance_level [0.0,1.5], advise 1.2
  double enhanceLevel;

  ZegoVideoEncoderEnhancementConfig(this.enhanceLevelMode, this.enhanceLevel);
}
...
class ZegoPublisherTakeSnapshotConfig {
  /// The position of taking snapshot. Default value: .
  ZegoPublisherTakeSnapshotPosition position;

  ZegoPublisherTakeSnapshotConfig(this.position);

  /// Constructs a publish take snapshot config object by default.
  ZegoPublisherTakeSnapshotConfig.defaultConfig()
      : position = ZegoPublisherTakeSnapshotPosition.AfterProcess;
}
...
abstract class ZegoRangeAudio {
  ...
  /// - Returns Error code, please refer to the error codes document https://www.zegocloud.com/docs/real-time-video-android-java/client-sdk/error-code for details.
...
abstract class ZegoCopyrightedMusic {
  ...
  /// When to call: After initializing the copyrighted music success [initCopyrightedMusic].
  ///
  /// - [krcToken] The krcToken obtained when calling [requestResource] for accompaniment or climax clips, or when obtaining shared resources through the [getSharedResource] interface. For more details, please refer to https://doc-zh.zego.im/online-ktv-ios/client-api/apis-to-obtain-songs-and-lyrics#2_2
...
abstract class ZegoScreenCaptureSource {
  ...
  /// When to call: It can be called after the engine by [createScreenCaptureSource] has been initialized.
  ///
  /// - [config] Screen capture parameter configuration, only available on the iOS, Android, and OHOS platform.
  ...
  ///
  /// - [visible] Whether to show the cursor. true to show the cursor, false to not show the cursor, the default is true.
...
abstract class ZegoPictureCapturer {
  ...
  ///
  /// - Returns Picture capturer instance index.
  int getIndex();
}

abstract class ZegoCustomAudioSource {
  /// Get custom audio source instance index.
  ///
  /// - Returns Custom audio source instance index.
  int getIndex();
  ...
  /// - Returns The result of the API call.
  Future<int> enableAEC(bool enable);
  ...
  /// Caution: ZegoCustomAudioSource can only be used as a mix source, not as the main capture source.
  ///
  /// - [volume] Volume, the range is 0 ~ 100.
  /// - Returns The result of the API call.
  Future<int> setVolume(int volume);
  ...
  /// Caution: ZegoCustomAudioSource can only be used as a mix source, not as the main capture source.
  ///
  /// - [mute] Whether to mute the custom audio source.
  /// - Returns The result of the API call.
  Future<int> mute(bool mute);
...
///
/// - [errorCode] Error code, please refer to the [Common Error Codes](https://www.zegocloud.com/docs/real-time-video-android-java/client-sdk/error-code) for details.
...
///
/// - [support] 0 - does not support the specified encoding capability, 1 - supports the specified encoding capability, 2 - undetermined.
class ZegoPublisherGetVideoEncoderSupportedResult {
  /// 0 - does not support the specified encoding capability, 1 - supports the specified encoding capability, 2 - undetermined.
  int support;

  ZegoPublisherGetVideoEncoderSupportedResult(this.support);
}
...
///
/// - [support] 0 - does not support the specified decoding capability, 1 - supports the specified decoding capability, 2 - undetermined.
class ZegoPlayerGetVideoDecoderSupportedResult {
  /// 0 - does not support the specified decoding capability, 1 - supports the specified decoding capability, 2 - undetermined.
  int support;

  ZegoPlayerGetVideoDecoderSupportedResult(this.support);
}
