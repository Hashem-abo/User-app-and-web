...
const String pluginVersion = "3.25.0-private.551";
...
class ZegoExpressImpl {
  ...
  static bool get handleCanvasView =>
      isEngineCreated ||
      (hostCanvasViewLifecycle != null && hostCanvasViewLifecycle!());
  ...
  static bool shouldUsePlatformView() {
    bool use = false;
    if (forceUsePlatformView != null) {
      use = forceUsePlatformView!();
    } else {
      use = _enablePlatformView;
    }
    ...
    if (kIsWindows &&
        ZegoExpressPlatformViewImpl.onWindowsPlatformViewCreated != null) {
      use &= kIsWindows;
    } else {
      use &= !kIsWindows;
    }
    ...
    if (kIsAndroid && _androidVersionCode > 0 && _androidVersionCode < 23) {
      try {
        String dartVersion = Platform.version.split(' ')[0];
        int? major = int.tryParse(dartVersion.split('.')[0]);
  ...
  static void Function(String message)? onCustomLogMessage;

  static bool Function()? hostCanvasViewLifecycle;

  static bool Function()? forceUsePlatformView;
  ...
  static Future<void> ensureAndroidVersionCode() async {
    if (kIsAndroid && _androidVersionCode == 0) {
      _androidVersionCode =
          await _channel.invokeMethod('getAndroidBuildVersionCode') ?? 0;
    }
  }
  ...
  static Future<void> createEngineWithProfile(ZegoEngineProfile profile) async {
    ...
    await _channel.invokeMethod('setPluginVersion', {'version': pluginVersion});
  }
  ...
  static Future<void> destroyEngine() async {
    ...
    if (hostCanvasViewLifecycle == null || !hostCanvasViewLifecycle!()) {
  ...
  Future<ZegoRoomLoginResult> loginRoom(String roomID, ZegoUser user,
    ...
    try {
      extendedDataMap = jsonDecode(map['extendedData']);
      // ignore: empty_catches
    } catch (e) {}
    return ZegoRoomLoginResult(map['errorCode'], extendedDataMap);
  }
  ...
  Future<ZegoRoomLogoutResult> logoutRoom([String? roomID]) async {
    ...
    return ZegoRoomLogoutResult(map['errorCode'], extendedDataMap);
  }
    ...
    return await _channel.invokeMethod('startPublishingStream', {
      ...
      'config': config != null
          ? {
              ...
              'streamCensorFlag': config.streamCensorFlag ?? 0,
  ...
  Future<void> startPreview(
    ...
    if (canvas != null && !shouldUsePlatformView()) {
      ...
      ZegoExpressTextureRenderer()
          .setRotation(canvas.view, canvas.rotation ?? 0);
    }
  ...
  Future<ZegoPublisherTakeSnapshotResult> takePublishStreamSnapshotByConfig(
      ZegoPublisherTakeSnapshotConfig config,
      {ZegoPublishChannel? channel}) async {
    final Map<dynamic, dynamic> map =
        await _channel.invokeMethod('takePublishStreamSnapshotByConfig', {
      'config': {'position': config.position.index},
      'channel': channel?.index ?? ZegoPublishChannel.Main.index
    });
  ...
  Future<ZegoPublisherGetVideoEncoderSupportedResult> getVideoEncoderSupported(
      ZegoVideoCodecID codecID,
      {ZegoVideoCodecBackend? codecBackend}) async {
    final Map<dynamic, dynamic> map =
        await _channel.invokeMethod('getVideoEncoderSupported', {
      'codecID': codecID.index,
      'codecBackend': codecBackend?.index ?? ZegoVideoCodecBackend.Any.index
    });

    return ZegoPublisherGetVideoEncoderSupportedResult(map['support']);
  }
  ...
  Future<void> enableVideoEncoderEnhancement(bool enable, double enhanceLevel,
      {ZegoPublishChannel? channel}) async {
    return await _channel.invokeMethod('enableVideoEncoderEnhancement', {
      'enable': enable,
  ...
  Future<void> enableVideoEncoderEnhancementWithConfig(
      bool enable, ZegoVideoEncoderEnhancementConfig config,
      {ZegoPublishChannel? channel}) async {
    return await _channel
        .invokeMethod('enableVideoEncoderEnhancementWithConfig', {
      'enable': enable,
      'config': {
        'enhanceLevelMode': config.enhanceLevelMode.index,
        'enhanceLevel': config.enhanceLevel,
      },
      'channel': channel?.index ?? ZegoPublishChannel.Main.index
    });
  }
  ...
  Future<void> startPlayingStream(String streamID,
    ...
    if (canvas != null && !shouldUsePlatformView()) {
      ...
      ZegoExpressTextureRenderer()
          .setRotation(canvas.view, canvas.rotation ?? 0);
      ZegoExpressTextureRenderer()
          .setMirror(canvas.view, canvas.mirror ?? false);
    }
    ...
    return await _channel.invokeMethod('switchPlayingStream', {
      ...
      'config': {
        ...
        'customResourceConfig': config.customResourceConfig != null
            ...
            : {},
        'switchStreamConfig': config.switchStreamConfig != null
            ? {
                'switchType': config.switchStreamConfig?.switchType.index,
                'switchTimeout': config.switchStreamConfig?.switchTimeout ?? 0,
              }
            : {},
      }
    });
  ...
  Future<void> switchPlayingResource(
    ...
    return await _channel.invokeMethod('switchPlayingResource',
        {'streamID': streamID, 'resourceType': resourceType.index});
  }
  ...
  Future<void> setPlayStreamDecodeFrameMode(
      String streamID, ZegoStreamDecodeMode frameMode) async {
    return await _channel.invokeMethod('setPlayStreamDecodeFrameMode',
        {'streamID': streamID, 'frameMode': frameMode.index});
  }
  ...
  Future<ZegoPlayerGetVideoDecoderSupportedResult> getVideoDecoderSupported(
      ...
      {ZegoVideoCodecBackend? codecBackend}) async {
    final Map<dynamic, dynamic> map =
        await _channel.invokeMethod('getVideoDecoderSupported', {
      'codecID': codecID.index,
      'codecBackend': codecBackend?.index ?? ZegoVideoCodecBackend.Any.index
    });

    return ZegoPlayerGetVideoDecoderSupportedResult(map['support']);
  }
  ...
  Future<int> setPlayingCanvas(String streamID, ZegoViewUpdateType updateType,
    ...
    return await _channel.invokeMethod('setPlayingCanvas', {
      'streamID': streamID,
      ...
      'canvas': canvas != null
          ? {
              ...
              'rotation': canvas.rotation ?? 0,
              'mirror': canvas.mirror ?? false,
              'viewContext': canvas.viewContext ?? ""
            }
          : {}
    });
  }
  ...
  Future<ZegoMixerStartResult> startMixerTask(ZegoMixerTask task) async {
    ...
    Map<dynamic, dynamic> extendedDataMap = {};
    try {
      extendedDataMap = jsonDecode(result['extendedData']);
      // ignore: empty_catches
    } catch (e) {}

    return ZegoMixerStartResult(result['errorCode'], extendedDataMap);
  }
  ...
  Future<void> startRecordingRemoteData(
    ...
    return await _channel.invokeMethod('startRecordingRemoteData', {
      'config': {
        'filePath': config.filePath,
        'recordType': config.recordType.index
      },
  ...
  Future<void> stopRecordingRemoteData(String streamID) async {
    return await _channel
        .invokeMethod('stopRecordingRemoteData', {'streamID': streamID});
  }
  ...
  Future<void> enableBeforeAudioPrepAudioData(
      bool enable, ZegoAudioFrameParam param) async {
    return await _channel.invokeMethod('enableBeforeAudioPrepAudioData', {
      'enable': enable,
      'param': {
        'sampleRate': param.sampleRate.value,
        'channel': param.channel.index,
  ...
  static final Map<int, ZegoCustomAudioSource> customAudioSourceMap = {};

  Future<ZegoCustomAudioSource?> createCustomAudioSource() async {
    int index = await _channel.invokeMethod('createCustomAudioSource');

    if (index >= 0) {
      ZegoCustomAudioSourceImpl audioSource = ZegoCustomAudioSourceImpl(index);
      customAudioSourceMap[index] = audioSource;
      return audioSource;
    }
  ...
  Future<void> destroyCustomAudioSource(
      ZegoCustomAudioSource audioSource) async {
    int index = audioSource.getIndex();
    await _channel.invokeMethod('destroyCustomAudioSource', {'index': index});
    customAudioSourceMap.remove(index);
    return;
  }
  ...
  Future<void> sendCustomAudioCaptureOpusData(Uint8List data, int dataLength,
      int referenceTimeMillisecond, int samples, ZegoAudioFrameParam param,
      {ZegoPublishChannel? channel}) async {
    return await _channel.invokeMethod('sendCustomAudioCaptureOpusData', {
      'data': data,
      'dataLength': dataLength,
      'referenceTimeMillisecond': referenceTimeMillisecond,
      'samples': samples,
  ...
  /* Picture Capturer */
  static final Map<int, ZegoPictureCapturer> pictureCapturerMap = {};
  Future<ZegoPictureCapturer?> createPictureCapturer() async {
    int index = await _channel.invokeMethod('createPictureCapturer');
    if (index >= 0) {
      ZegoPictureCapturer pictureCapturerInstance =
          ZegoPictureCapturerImpl(index);
      pictureCapturerMap[index] = pictureCapturerInstance;
      return pictureCapturerInstance;
    } else {
      return null;
    }
  }
  ...
  Future<void> destroyPictureCapturer(
    ...
    await _channel.invokeMethod('destroyPictureCapturer', {'index': index});
    pictureCapturerMap.remove(index);
  }
  ...
  static void _eventListener(dynamic data) {
    ...
    switch (map['method']) {
      ...
      case 'onPublisherQualityUpdate':
        ...
        ZegoExpressEngine.onPublisherQualityUpdate!(
            ...
            ZegoPublishStreamQuality(
              ...
              map['quality']['audioTrafficControlRate'],
              map['quality']['videoTrafficControlRate'],
            ));
        break;
      ...
      case 'onPublisherVideoSizeChanged':
        if (kIsAndroid && !ZegoExpressImpl.shouldUsePlatformView()) {
          final int? textureID = map['textureID'];
          if (textureID != null) {
            ZegoExpressTextureRenderer()
                .updateRendererSize(textureID, map['width'], map['height']);
          }
        }
      ...
      case 'onPublisherFaceDetectInfo':
        ...
        faceDetectionInfo.errorCode = map['info']['errorCode'];
      ...
      case 'onPlayerRenderMultiViewFirstFrame':
        if (ZegoExpressEngine.onPlayerRenderMultiViewFirstFrame == null) return;

        ZegoExpressEngine.onPlayerRenderMultiViewFirstFrame!(
            map['streamID'], map['viewContext']);
        break;
      ...
      case 'onRemoteDataRecordStateUpdate':
        if (ZegoExpressEngine.onRemoteDataRecordStateUpdate == null) return;

        ZegoExpressEngine.onRemoteDataRecordStateUpdate!(
            ZegoDataRecordState.values[map['state']],
            map['errorCode'],
            ZegoDataRecordConfig(map['config']['filePath'],
                ZegoDataRecordType.values[map['config']['recordType']]),
      ...
      case 'onRemoteDataRecordProgressUpdate':
        if (ZegoExpressEngine.onRemoteDataRecordProgressUpdate == null) return;

        ZegoPlayStreamQuality quality = ZegoPlayStreamQuality(
            map['quality']['videoRecvFPS'],
            map['quality']['videoDejitterFPS'],
            map['quality']['videoDecodeFPS'],
            map['quality']['videoRenderFPS'],
        ...
        ZegoExpressEngine.onRemoteDataRecordProgressUpdate!(
            ZegoDataRemoteRecordProgress(map['progress']['duration'],
                map['progress']['currentFileSize'], quality),
            ZegoDataRecordConfig(map['config']['filePath'],
                ZegoDataRecordType.values[map['config']['recordType']]),
            map['streamID']);
...
class ZegoCustomAudioSourceImpl extends ZegoCustomAudioSource {
  final int _index;

  ZegoCustomAudioSourceImpl(int index) : _index = index;
  ...
  Future<int> startCapture() async {
    return await ZegoExpressImpl._channel
        .invokeMethod('customAudioSourceStartCapture', {'index': _index});
  }

  @override
  Future<int> stopCapture() async {
    return await ZegoExpressImpl._channel
        .invokeMethod('customAudioSourceStopCapture', {'index': _index});
  }

  @override
  Future<int> enableAEC(bool enable) async {
    return await ZegoExpressImpl._channel.invokeMethod(
        'customAudioSourceEnableAEC', {'index': _index, 'enable': enable});
  }

  @override
  Future<int> setVolume(int volume) async {
    return await ZegoExpressImpl._channel.invokeMethod(
        'customAudioSourceSetVolume', {'index': _index, 'volume': volume});
  }

  @override
  Future<int> mute(bool mute) async {
    return await ZegoExpressImpl._channel
        .invokeMethod('customAudioSourceMute', {'index': _index, 'mute': mute});
  }

  @override
  Future<int> pushAudioFrame(
      Uint8List data, int dataLength, ZegoAudioFrameParam param) async {
    return await ZegoExpressImpl._channel
        .invokeMethod('customAudioSourcePushAudioFrame', {
      'index': _index,
      'data': data,
      'dataLength': dataLength,
      'param': {
...
class ZegoScreenCaptureSourceImpl extends ZegoScreenCaptureSource {
      ...
      'config': config == null
          ...
          : {
              ...
              'orientation': config.orientation?.index ??
                  ZegoScreenCaptureOrientation.Auto.index,
              'muteExtensMicrophone': config.muteExtensMicrophone ?? false,
              'audioDeviceMode': config.audioDeviceMode?.index ??
                  ZegoScreenCaptureAudioDeviceMode.None.index
            },
      'inApp': inApp,
      'index': _index
    });
  ...
  Future<void> enableAudioCaptureWithConfig(bool enable,
      {ZegoScreenCaptureAudioConfig? config}) async {
    return await ZegoExpressImpl._channel
        .invokeMethod('enableAudioCaptureWithConfigScreenCaptureSource', {
      'enable': enable,
      'config': {
        'enableWindowCapture': config?.enableWindowCapture ?? false,
        'excludeProcessID': config?.excludeProcessID ?? 0,
      },
      'index': _index
    });
  }
...
class ZegoPictureCapturerImpl extends ZegoPictureCapturer {
  final int _index;

  ZegoPictureCapturerImpl(int index) : _index = index;
  ...
  Future<void> setPath(String path) async {
    return await ZegoExpressImpl._channel.invokeMethod(
        'setPathPictureCapturer', {'index': _index, 'path': path});
  }
}
