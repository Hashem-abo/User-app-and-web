...
#define ZEGO_CHECK_ENGINE(result)                                                  \
    do {                                                                           \
        if (!EXPRESS::ZegoExpressSDK::getEngine()) {                               \
            ZF::logInfo("engine is not created, please call createEngine first, "  \
                        "call ignored");                                            \
            (result)->Error("engine_not_created",                                  \
                            "Engine is not created, please call createEngine "      \
                            "first");                                               \
            return;                                                                \
        }                                                                          \
    } while (0)
...
#ifdef _WIN32
        enablePlatformView_ =
        zego_value_get_bool(argument[ZFValue("enablePlatformView")]);
#endif
...
#ifdef _WIN32
        enablePlatformView_ =
        zego_value_get_bool(profileMap[ZFValue("enablePlatformView")]);
#endif
...
void ZegoExpressEngineMethodHandler::destroyEngine(ZFArgument argument, ZFResult result) {
    ...
    if (engine) {
        ...
        // Detach the audio-related handlers before tearing the engine down.
        // These callbacks fire on an SDK internal thread; nulling them
        // guarantees they can no longer reach the event handler during engine
        // destruction, eliminating the race that crashed SendPlatformMessage.
        engine->setAudioDataHandler(nullptr);
        engine->setCustomAudioProcessHandler(nullptr);
        engine->setDataRecordEventHandler(nullptr);
        // Detach the engine's custom-video-render callback before tearing down
        // the texture controller, so the engine's frame thread cannot race the
        // raster thread that uploads those buffers to GL.
        engine->setCustomVideoRenderHandler(nullptr);
...
void ZegoExpressEngineMethodHandler::setDummyCaptureImageParams(ZFArgument argument,
                                                                ZFResult result) {
    ZEGO_CHECK_ENGINE(result);
    ...
    params.mode =
        (EXPRESS::ZegoDummyCaptureImageMode)zego_value_get_int(paramsMap[ZFValue("mode")]);
    const std::string flutterAssertTaget = "flutter-asset://";
    ...
    if (configMap.size() > 0) {
        ...
        if (!zego_value_is_null(configMap[ZFValue("capabilityNegotiationTypes")])) {
            config.capabilityNegotiationTypes =
                zego_value_get_long(configMap[ZFValue("capabilityNegotiationTypes")]);
        }
        if (!zego_value_is_null(configMap[ZFValue("roomType")])) {
            config.roomType = zego_value_get_long(configMap[ZFValue("roomType")]);
        }
    }
    auto sharedPtrResult = ZFMoveResult(result);
...
void ZegoExpressEngineMethodHandler::startPublishingStream(ZFArgument argument, ZFResult result) {
    ...
    if (!zego_value_is_null(argument[ZFValue("config")])) {
        ...
        if (configMap.size() > 0) {
            ...
            config.streamCensorFlag = zego_value_get_int(configMap[ZFValue("streamCensorFlag")]);
            config.codecNegotiationType = (EXPRESS::ZegoCapabilityNegotiationType)zego_value_get_int(
                configMap[ZFValue("codecNegotiationType")]);
...
void ZegoExpressEngineMethodHandler::startPreview(ZFArgument argument, ZFResult result) {
    ...
    if (EXPRESS::ZegoExpressSDK::getEngine() && canvasMap.size() > 0) {
        ...
        int backgroundColor =
            zego_value_get_int(canvasMap[ZFValue("backgroundColor")]);
        if (!enablePlatformView_) {
            ...
            canvas.view = (void *)viewID;
            canvas.alphaBlend = alphaBlend;
            canvas.backgroundColor = backgroundColor;
        }
    }
    EXPRESS::ZegoExpressSDK::getEngine()->startPreview(&canvas,
                                                       (EXPRESS::ZegoPublishChannel)channel);
    result->Success();
}
...
void ZegoExpressEngineMethodHandler::setVideoConfig(ZFArgument argument, ZFResult result) {
    ...
    config.codecID =
        (EXPRESS::ZegoVideoCodecID)(codecIDIndex > 4
                                        ? EXPRESS::ZegoVideoCodecID::ZEGO_VIDEO_CODEC_ID_UNKNOWN
                                        : codecIDIndex);
...
void ZegoExpressEngineMethodHandler::takePublishStreamSnapshotByConfig(ZFArgument argument, ZFResult result) {
    ZEGO_CHECK_ENGINE(result);

    auto channel = zego_value_get_int(argument[ZFValue("channel")]);
    auto configMap = zego_value_get_map(argument[ZFValue("config")]);
    EXPRESS::ZegoPublisherTakeSnapshotConfig config;
    config.position = (EXPRESS::ZegoPublisherTakeSnapshotPosition)zego_value_get_int(configMap[ZFValue("position")]);
    
    auto sharedPtrResult = ZFMoveResult(result);
    EXPRESS::ZegoExpressSDK::getEngine()->takePublishStreamSnapshotByConfig(config,
        [=](int errorCode, void *image) {
            std::vector<uint8_t> raw_image;
            if (image) {
#ifdef _WIN32
...
void ZegoExpressEngineMethodHandler::setStreamAlignmentProperty(ZFArgument argument,
                                                                ZFResult result) {
    ZEGO_CHECK_ENGINE(result);

    auto alignment = zego_value_get_int(argument[ZFValue("alignment")]);
    auto channel = zego_value_get_int(argument[ZFValue("channel")]);

    EXPRESS::ZegoExpressSDK::getEngine()->setStreamAlignmentProperty(
        alignment, (EXPRESS::ZegoPublishChannel)channel);
...
void ZegoExpressEngineMethodHandler::getVideoEncoderSupported(ZFArgument argument,
                                                              ZFResult result) {
    ZEGO_CHECK_ENGINE(result);
    ...
    auto sharedPtrResult = ZFMoveResult(result);
    EXPRESS::ZegoExpressSDK::getEngine()->getVideoEncoderSupported(
        (EXPRESS::ZegoVideoCodecID)codecID, (EXPRESS::ZegoVideoCodecBackend)codecBackend,
        [=](int support) {
            ZFMap retMap;
            retMap[ZFValue("support")] = ZFValue(support);
            sharedPtrResult->Success(retMap);
        });
}
...
void ZegoExpressEngineMethodHandler::setAudioSource(ZFArgument argument, ZFResult result) {
    ...
    if (!zego_value_is_null(argument[ZFValue("config")])) {
        ...
        auto audioSourceIndexList = zego_value_get_list(configMap[ZFValue("audioSourceIndexList")]);
        config.audioSourceCount = audioSourceIndexList.size();
        int *audioSources = nullptr;
        if (config.audioSourceCount > 0) {
            audioSources = new int[config.audioSourceCount];
        }
        config.audioSourceIndexList = audioSources;
        for (auto audioSourceIndex : audioSourceIndexList) {
            *audioSources = zego_value_get_int(audioSourceIndex);
            audioSources += 1;
        }
    ...
    if (!hasChannel && !hasConfig) {
        ...
        delete[] config.audioSourceIndexList;
    } else {
        if (channel == 0) {
            ret = EXPRESS::ZegoExpressSDK::getEngine()->setAudioSource(
                (EXPRESS::ZegoAudioSourceType)source, config);
...
void ZegoExpressEngineMethodHandler::enableVideoEncoderEnhancement(ZFArgument argument,
                                                                    ZFResult result) {
    ZEGO_CHECK_ENGINE(result);

    auto enable = zego_value_get_bool(argument[ZFValue("enable")]);
    auto channel = zego_value_get_int(argument[ZFValue("channel")]);
    auto enhanceLevel = zego_value_get_double(argument[ZFValue("enhanceLevel")]);

    EXPRESS::ZegoExpressSDK::getEngine()->enableVideoEncoderEnhancement(enable, (float)enhanceLevel, (EXPRESS::ZegoPublishChannel)channel);

    result->Success();
}

void ZegoExpressEngineMethodHandler::enableVideoEncoderEnhancementWithConfig(ZFArgument argument,
                                                                    ZFResult result) {
    ZEGO_CHECK_ENGINE(result);
    ...
    EXPRESS::ZegoVideoEncoderEnhancementConfig config;
    if (!zego_value_is_null(argument[ZFValue("config")])) {
        auto configMap = zego_value_get_map(argument[ZFValue("config")]);
        config.enhanceLevel = zego_value_get_double(configMap[ZFValue("enhanceLevel")]);
        config.enhanceLevelMode = (EXPRESS::ZegoVideoEncodeEnhanceLevelMode)zego_value_get_int(configMap[ZFValue("enhanceLevelMode")]);
    }

    EXPRESS::ZegoExpressSDK::getEngine()->enableVideoEncoderEnhancementWithConfig(enable, config, (EXPRESS::ZegoPublishChannel)channel);
...
void ZegoExpressEngineMethodHandler::enableFaceDetection(ZFArgument argument, ZFResult result) {
    // auto enable = zego_value_get_bool(argument[ZFValue("enable")]);
    // auto channel = zego_value_get_int(argument[ZFValue("channel")]);
    ...
    if (configMap.size() > 0) {
        ...
        if (!zego_value_is_null(configMap[ZFValue("cdnConfig")])) {
            ...
            if (cdnConfigMap.size() > 0) {
                ...
                cdnConfigPtr->customParams =
                    zego_value_get_string(cdnConfigMap[ZFValue("customParams")]);
        ...
        if (!zego_value_is_null(configMap[ZFValue("customResourceConfig")])) {
            auto customResourceConfigMap =
                zego_value_get_map(configMap[ZFValue("customResourceConfig")]);
            if (customResourceConfigMap.size() > 0) {
                config.customResourceConfig.beforePublish =
                    (EXPRESS::ZegoResourceType)zego_value_get_int(
                        customResourceConfigMap[ZFValue("beforePublish")]);
                config.customResourceConfig.publishing =
                    (EXPRESS::ZegoResourceType)zego_value_get_int(
                        customResourceConfigMap[ZFValue("publishing")]);
                config.customResourceConfig.afterPublish =
                    (EXPRESS::ZegoResourceType)zego_value_get_int(
                        customResourceConfigMap[ZFValue("afterPublish")]);
            }
        }

        EXPRESS::ZegoExpressSDK::getEngine()->startPlayingStream(streamID, &canvas, config);
    } else {
        EXPRESS::ZegoExpressSDK::getEngine()->startPlayingStream(streamID, &canvas);
    }
...
void ZegoExpressEngineMethodHandler::switchPlayingStream(ZFArgument argument, ZFResult result) {
    ...
    if (configMap.size() > 0) {
        ...
        if (!zego_value_is_null(configMap[ZFValue("switchStreamConfig")])) {
            auto switchStreamConfigMap = zego_value_get_map(configMap[ZFValue("switchStreamConfig")]);
            if (switchStreamConfigMap.size() > 0) {
                config.switchStreamConfig.switchType = (EXPRESS::ZegoSwitchPlayingStreamType)zego_value_get_int(switchStreamConfigMap[ZFValue("switchType")]);
                config.switchStreamConfig.switchTimeout = (int)zego_value_get_int(switchStreamConfigMap[ZFValue("switchTimeout")]);
            }
        }
    } 
...
void ZegoExpressEngineMethodHandler::switchPlayingResource(ZFArgument argument, ZFResult result) {
    ZEGO_CHECK_ENGINE(result);

    auto streamID = zego_value_get_string(argument[ZFValue("streamID")]);
    auto resourceType =
        (EXPRESS::ZegoResourceType)zego_value_get_int(argument[ZFValue("resourceType")]);

    EXPRESS::ZegoExpressSDK::getEngine()->switchPlayingResource(streamID, resourceType);
...
void ZegoExpressEngineMethodHandler::mediaPlayerEnableVoiceChanger(ZFArgument argument,
    ...
    if (mediaPlayer) {
        ...
        EXPRESS::ZegoVoiceChangerParam param;
        param.pitch = (float)pitch;
        mediaPlayer->enableVoiceChanger((EXPRESS::ZegoMediaPlayerAudioChannel)audioChannel, enable,
                                        param);
    }
    ...
    if (mediaPlayer) {
        ...
        if (!enablePlatformView_) {
            ...
            mediaPlayer->takeSnapshot([=](int errorCode, void *image) {
                ZFMap retMap;
                retMap[ZFValue("errorCode")] = ZFValue(errorCode);
                retMap[ZFValue("image")] = ZFValue(ConvertHBITMAPToVector((HBITMAP)image));
                sharedPtrResult->Success(retMap);
            });
            return;
        }
...
void ZegoExpressEngineMethodHandler::mediaPlayerLoadResourceWithConfig(ZFArgument argument,
    ...
    if (mediaPlayer) {
        ...
        resource.onlineResourceCachePath =
            zego_value_get_string(resourceMap[ZFValue("onlineResourceCachePath")]);
        resource.maxCachePendingLength =
            zego_value_get_long(resourceMap[ZFValue("maxCachePendingLength")]);
...
void ZegoExpressEngineMethodHandler::startMixerTask(ZFArgument argument, ZFResult result) {
    ...
    for (const auto &inputIter : inputFlutterList) {
        ...
        if(!zego_value_is_null(inputMap[ZFValue("blurInfo")])) {
            auto blurInfoMap = zego_value_get_map(inputMap[ZFValue("blurInfo")]);
            if (!zego_value_is_null(blurInfoMap[ZFValue("topPadding")])) {
                input.blurInfo.topPadding = zego_value_get_int(blurInfoMap[ZFValue("topPadding")]);
            }
            
            if (!zego_value_is_null(blurInfoMap[ZFValue("leftPadding")])) {
                input.blurInfo.leftPadding = zego_value_get_int(blurInfoMap[ZFValue("leftPadding")]);
            }
            
            if (!zego_value_is_null(blurInfoMap[ZFValue("bottomPadding")])) {
                input.blurInfo.bottomPadding = zego_value_get_int(blurInfoMap[ZFValue("bottomPadding")]);
            }
            
            if (!zego_value_is_null(blurInfoMap[ZFValue("rightPadding")])) {
                input.blurInfo.rightPadding = zego_value_get_int(blurInfoMap[ZFValue("rightPadding")]);
            }
        }
        ...
        if (whiteboardID != 0) {
            ...
            if (!zego_value_is_null(whiteboardMap[ZFValue("backgroundColor")])) {
                whiteboard.backgroundColor =
                    zego_value_get_int(whiteboardMap[ZFValue("backgroundColor")]);
            }
...
void ZegoExpressEngineMethodHandler::enableAudioCaptureDevice(ZFArgument argument,
    ...
    auto sharedPtrResult = ZFMoveResult(result);
    EXPRESS::ZegoExpressSDK::getEngine()->enableAudioCaptureDeviceAsync(enable, [=](int errorCode){
...
void ZegoExpressEngineMethodHandler::startRecordingRemoteData(ZFArgument argument,
                                                                ZFResult result) {
    ZEGO_CHECK_ENGINE(result);
    ...
    EXPRESS::ZegoExpressSDK::getEngine()->startRecordingRemoteData(streamID, config);
    result->Success();
}

void ZegoExpressEngineMethodHandler::stopRecordingRemoteData(ZFArgument argument,
                                                               ZFResult result) {
    ZEGO_CHECK_ENGINE(result);

    auto streamID = zego_value_get_string(argument[ZFValue("streamID")]);

    EXPRESS::ZegoExpressSDK::getEngine()->stopRecordingRemoteData(streamID);
    result->Success();
}
...
void ZegoExpressEngineMethodHandler::getVideoDecoderSupported(ZFArgument argument,
                                                              ZFResult result) {
    ZEGO_CHECK_ENGINE(result);
    ...
    auto sharedPtrResult = ZFMoveResult(result);
    EXPRESS::ZegoExpressSDK::getEngine()->getVideoDecoderSupported(
        (EXPRESS::ZegoVideoCodecID)codecID, (EXPRESS::ZegoVideoCodecBackend)codecBackend,
        [=](int support) {
            ZFMap retMap;
            retMap[ZFValue("support")] = ZFValue(support);
...
void ZegoExpressEngineMethodHandler::setPlayStreamDecodeFrameMode(ZFArgument argument,
                                                                     ZFResult result) {
    ZEGO_CHECK_ENGINE(result);

    auto frameMode = zego_value_get_int(argument[ZFValue("frameMode")]);
    auto streamID = zego_value_get_string(argument[ZFValue("streamID")]);

    EXPRESS::ZegoExpressSDK::getEngine()->setPlayStreamDecodeFrameMode(streamID,
        (EXPRESS::ZegoStreamDecodeMode)frameMode);
...
void ZegoExpressEngineMethodHandler::updatePlayingCanvas(ZFArgument argument, ZFResult result) {
    ...
    if (EXPRESS::ZegoExpressSDK::getEngine() && canvasMap.size() > 0) {
        ...
        if (!enablePlatformView_) {
            ...
            canvas.alphaBlend = alphaBlend;
            canvas.backgroundColor = backgroundColor;
            isSuccess = EXPRESS::ZegoExpressSDK::getEngine()->updatePlayingCanvas(
                streamID, &canvas);
        }
    }
...
void ZegoExpressEngineMethodHandler::setPlayingCanvas(ZFArgument argument, ZFResult result) {
    result->Error("not_support_feature", "windows platform not support feature");
}
...
void ZegoExpressEngineMethodHandler::startAutoMixerTask(ZFArgument argument, ZFResult result) {
    ...
    task.streamAlignmentVolumeControlMode = (EXPRESS::ZegoStreamAlignmentVolumeControlMode)zego_value_get_int(argument[ZFValue("streamAlignmentVolumeControlMode")]);
    task.streamAlignmentBaselineStreamID = zego_value_get_string(argument[ZFValue("streamAlignmentBaselineStreamID")]);
...
void ZegoExpressEngineMethodHandler::enableCustomVideoRender(ZFArgument argument, ZFResult result) {
    ...
    if (!enablePlatformView_) {
        ...
        if (enable) {
            ...
            EXPRESS::ZegoExpressSDK::getEngine()->setCustomVideoRenderHandler(nullptr);
        }
        EXPRESS::ZegoExpressSDK::getEngine()->enableCustomVideoRender(enable, &config);
    }
...
void ZegoExpressEngineMethodHandler::enableBeforeAudioPrepAudioData(ZFArgument argument, ZFResult result) {
    ZEGO_CHECK_ENGINE(result);

    auto enable = zego_value_get_bool(argument[ZFValue("enable")]);
    auto paramMap = zego_value_get_map(argument[ZFValue("param")]);
    EXPRESS::ZegoAudioFrameParam param;
    param.sampleRate = (EXPRESS::ZegoAudioSampleRate)zego_value_get_int(paramMap[ZFValue("sampleRate")]);
    param.channel = (EXPRESS::ZegoAudioChannel)zego_value_get_int(paramMap[ZFValue("channel")]);

    EXPRESS::ZegoExpressSDK::getEngine()->enableBeforeAudioPrepAudioData(enable, param);
...
void ZegoExpressEngineMethodHandler::createCustomAudioSource(ZFArgument argument,
                                                             ZFResult result) {
    ZEGO_CHECK_ENGINE(result);

    auto audioSource = EXPRESS::ZegoExpressSDK::getEngine()->createCustomAudioSource();
    if (audioSource) {
        auto index = audioSource->getIndex();
        customAudioSourceMap_[index] = audioSource;
        result->Success(ZFValue(index));
    } else {
        result->Success(ZFValue(-1));
    }
...
void ZegoExpressEngineMethodHandler::destroyCustomAudioSource(ZFArgument argument,
                                                              ZFResult result) {
    ZEGO_CHECK_ENGINE(result);

    auto index = zego_value_get_int(argument[ZFValue("index")]);
    auto audioSource = customAudioSourceMap_[index];

    if (audioSource) {
        EXPRESS::ZegoExpressSDK::getEngine()->destroyCustomAudioSource(audioSource);
    }
    customAudioSourceMap_.erase(index);
    result->Success();
}

void ZegoExpressEngineMethodHandler::customAudioSourceStartCapture(ZFArgument argument,
                                                                   ZFResult result) {
    auto index = zego_value_get_int(argument[ZFValue("index")]);
    auto audioSource = customAudioSourceMap_[index];
    result->Success(ZFValue(audioSource ? audioSource->startCapture() : -1));
}

void ZegoExpressEngineMethodHandler::customAudioSourceStopCapture(ZFArgument argument,
                                                                  ZFResult result) {
    auto index = zego_value_get_int(argument[ZFValue("index")]);
    auto audioSource = customAudioSourceMap_[index];
    result->Success(ZFValue(audioSource ? audioSource->stopCapture() : -1));
}

void ZegoExpressEngineMethodHandler::customAudioSourceEnableAEC(ZFArgument argument,
                                                                ZFResult result) {
    auto index = zego_value_get_int(argument[ZFValue("index")]);
    auto enable = zego_value_get_bool(argument[ZFValue("enable")]);
    auto audioSource = customAudioSourceMap_[index];
    result->Success(ZFValue(audioSource ? audioSource->enableAEC(enable) : -1));
}

void ZegoExpressEngineMethodHandler::customAudioSourceSetVolume(ZFArgument argument,
                                                                ZFResult result) {
    auto index = zego_value_get_int(argument[ZFValue("index")]);
    auto volume = zego_value_get_int(argument[ZFValue("volume")]);
    auto audioSource = customAudioSourceMap_[index];
    result->Success(ZFValue(audioSource ? audioSource->setVolume(volume) : -1));
}

void ZegoExpressEngineMethodHandler::customAudioSourceMute(ZFArgument argument, ZFResult result) {
    auto index = zego_value_get_int(argument[ZFValue("index")]);
    auto mute = zego_value_get_bool(argument[ZFValue("mute")]);
    auto audioSource = customAudioSourceMap_[index];
    result->Success(ZFValue(audioSource ? audioSource->mute(mute) : -1));
}

void ZegoExpressEngineMethodHandler::customAudioSourcePushAudioFrame(ZFArgument argument,
                                                                     ZFResult result) {
    auto index = zego_value_get_int(argument[ZFValue("index")]);
    auto byteData = zego_value_get_vector_uint8(argument[ZFValue("data")]);
    ...
    auto audioSource = customAudioSourceMap_[index];
    result->Success(
        ZFValue(audioSource ? audioSource->pushAudioFrame(byteData.data(), dataLength, param) : -1));
}
...
void ZegoExpressEngineMethodHandler::sendCustomAudioCaptureAACData(ZFArgument argument,
    ...
    auto referenceTimeMillisecond =
        zego_value_get_long(argument[ZFValue("referenceTimeMillisecond")]);
...
void ZegoExpressEngineMethodHandler::sendCustomAudioCaptureOpusData(ZFArgument argument,
                                                                   ZFResult result) {
    ZEGO_CHECK_ENGINE(result);
    ...
    EXPRESS::ZegoExpressSDK::getEngine()->sendCustomAudioCaptureOpusData(
        byteData.data(), dataLength, (unsigned long long)referenceTimeMillisecond,
        (unsigned int)samples, param,
        (EXPRESS::ZegoPublishChannel)channel);
...
bool ZegoExpressEngineMethodHandler::isScreenCaptureSourcesValid() {
    return !screenCaptureSourceMap_.empty();
}
...
void ZegoExpressEngineMethodHandler::destroyScreenCaptureSource(ZFArgument argument,
    ...
    if (screenCaptureSource) {
        screenCaptureSource->setEventHandler(nullptr);
...
void ZegoExpressEngineMethodHandler::enableAudioCaptureWithConfigScreenCaptureSource(ZFArgument argument, ZFResult result) {
#ifdef _WIN32
    auto index = zego_value_get_int(argument[ZFValue("index")]);
    auto screenCaptureSource = screenCaptureSourceMap_[index];
    ...
    if (screenCaptureSource) {
        ...
        EXPRESS::ZegoScreenCaptureAudioConfig nativeConfig;
        nativeConfig.enableWindowCapture = zego_value_get_bool(configMap[ZFValue("enableWindowCapture")]);
        nativeConfig.excludeProcessID = zego_value_get_long(configMap[ZFValue("excludeProcessID")]);

        screenCaptureSource->enableAudioCapture(enable, nativeConfig);
    }
...
// Picture Capturer
void ZegoExpressEngineMethodHandler::createPictureCapturer(ZFArgument argument, ZFResult result) {
    ZEGO_CHECK_ENGINE(result);

    auto pictureCapturer = EXPRESS::ZegoExpressSDK::getEngine()->createPictureCapturer();
    if (pictureCapturer) {
        auto index = pictureCapturer->getIndex();
        pictureCapturerMap_[index] = pictureCapturer;
        result->Success(ZFValue(index));
    } else {
        result->Success(ZFValue(-1));
    }
...
void ZegoExpressEngineMethodHandler::destroyPictureCapturer(ZFArgument argument, ZFResult result) {
    ZEGO_CHECK_ENGINE(result);

    auto index = zego_value_get_int(argument[ZFValue("index")]);
    auto pictureCapturer = pictureCapturerMap_[index];

    if (pictureCapturer) {
        EXPRESS::ZegoExpressSDK::getEngine()->destroyPictureCapturer(pictureCapturer);
    }

    pictureCapturerMap_.erase(index);

    result->Success();
}

void ZegoExpressEngineMethodHandler::setPathPictureCapturer(ZFArgument argument, ZFResult result) {
    auto index = zego_value_get_int(argument[ZFValue("index")]);
    auto pictureCapturer = pictureCapturerMap_[index];

    if (pictureCapturer) {
        auto path = zego_value_get_string(argument[ZFValue("path")]);
        pictureCapturer->setPath(path);
    }

    result->Success();
}

void ZegoExpressEngineMethodHandler::uninitModules() {
    for (auto &audioSource : customAudioSourceMap_) {
        if (audioSource.second) {
            EXPRESS::ZegoExpressSDK::getEngine()->destroyCustomAudioSource(audioSource.second);
        }
    }
    customAudioSourceMap_.clear();

    for (auto &screenScource : screenCaptureSourceMap_) {
        if (screenScource.second) {
            screenScource.second->setEventHandler(nullptr);
            EXPRESS::ZegoExpressSDK::getEngine()->destroyScreenCaptureSource(screenScource.second);
        }
    }
    screenCaptureSourceMap_.clear();
}
