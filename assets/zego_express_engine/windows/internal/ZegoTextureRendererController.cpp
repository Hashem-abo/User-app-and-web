...
bool ZegoTextureRendererController::convertYUVToRGBA(const uint8_t *const *data,
                                                     const uint32_t *dataLength,
                                                     ZegoVideoFrameParam &param,
                                                     std::vector<uint8_t> &rgbaBuffer) {
    const int w = param.width;
    const int h = param.height;
    const int yStride = param.strides[0] > 0 ? param.strides[0] : w;

    // BT.601 limited-range YUV -> full-range RGB, integer math.
    auto clip8 = [](int v) -> uint8_t {
        return v < 0 ? 0 : (v > 255 ? 255 : static_cast<uint8_t>(v));
    };

    const uint32_t rgbaSize = static_cast<uint32_t>(w) * h * 4;
    if (rgbaBuffer.size() != rgbaSize) {
        rgbaBuffer.resize(rgbaSize);
    }
    auto *dst = reinterpret_cast<FlutterDesktopPixel *>(rgbaBuffer.data());

    switch (param.format) {
    case ZEGO_VIDEO_FRAME_FORMAT_I420: {
        const uint8_t *yPlane = data[0];
        const uint8_t *uPlane = data[1];
        const uint8_t *vPlane = data[2];
        const int uStride = param.strides[1] > 0 ? param.strides[1] : w / 2;
        const int vStride = param.strides[2] > 0 ? param.strides[2] : w / 2;
        for (int j = 0; j < h; j++) {
            for (int i = 0; i < w; i++) {
                const int Y = yPlane[j * yStride + i];
                const int U = uPlane[(j / 2) * uStride + (i / 2)];
                const int V = vPlane[(j / 2) * vStride + (i / 2)];
                const int c = Y - 16, d = U - 128, e = V - 128;
                dst[j * w + i] = {clip8((298 * c + 516 * d + 128) >> 8),
                                  clip8((298 * c - 100 * d - 208 * e + 128) >> 8),
                                  clip8((298 * c + 409 * e + 128) >> 8), 255};
            }
        }
        break;
    }
    case ZEGO_VIDEO_FRAME_FORMAT_NV12:
    case ZEGO_VIDEO_FRAME_FORMAT_NV21: {
        const uint8_t *yPlane = data[0];
        const uint8_t *uvPlane = data[1];
        const int uvStride = param.strides[1] > 0 ? param.strides[1] : w;
        const bool isNV21 = (param.format == ZEGO_VIDEO_FRAME_FORMAT_NV21);
        for (int j = 0; j < h; j++) {
            for (int i = 0; i < w; i++) {
                const int Y = yPlane[j * yStride + i];
                const int uvIdx = (j / 2) * uvStride + ((i / 2) * 2);
                const int U = isNV21 ? uvPlane[uvIdx + 1] : uvPlane[uvIdx];
                const int V = isNV21 ? uvPlane[uvIdx] : uvPlane[uvIdx + 1];
                const int c = Y - 16, d = U - 128, e = V - 128;
                dst[j * w + i] = {clip8((298 * c + 516 * d + 128) >> 8),
                                  clip8((298 * c - 100 * d - 208 * e + 128) >> 8),
                                  clip8((298 * c + 409 * e + 128) >> 8), 255};
    ...
    default:
        // Not a YUV format we handle.
        return false;
    }

    param.format = ZEGO_VIDEO_FRAME_FORMAT_RGBA32;
    return true;
}
...
void ZegoTextureRendererController::onCapturedVideoFrameRawData(uint8_t **data,
                                                                ...
                                                                ZegoPublishChannel channel) {
    {
        ...
        if (renderer != capturedRenderers_.end()) {
            ...
            ZegoVideoFrameParam rgbaParam = param;
            if (convertYUVToRGBA(data, dataLength, rgbaParam, yuvConvertBuffer_)) {
                renderer->second->updateSrcFrameBuffer(yuvConvertBuffer_.data(),
                                                       yuvConvertBuffer_.size(), rgbaParam);
            } else {
