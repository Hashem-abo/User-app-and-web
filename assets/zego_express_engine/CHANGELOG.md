...
1. **Supports inputting Opus-encoded audio data through custom audio capture - Android/iOS/macOS/Windows**

    - Relevate APIs: `sendCustomAudioCaptureOpusData`

2. **Supports switching playback resources between L3 (Live Streaming) and RTC - Android/iOS/Windows**

    - The switching flow has been optimized for smoother playback.

    - Relevate APIs: `switchPlayingResource`

3. **Supports combining custom audio capture with custom screen capture - Android/iOS/Windows**

    - Suitable for multi-channel stream publishing and channel switching scenarios.

    - Relevate APIs: `createCustomAudioSource` `destroyCustomAudioSource`

4. **Copyrighted music scoring supports memory resource mode - Android/iOS/Windows/Web**

    - Supports scenarios where resources are loaded and played from memory.

    - Relevate APIs: `loadResourceFromMediaData`

### Improvements & Optimizations

1. **Reduced the number of TLS keys created by the SDK, lowering resource usage - All Platforms**

2. **Increased the default maximum number of stream playback from 12 to 20 - All Platforms**

3. **Optimized word accuracy in low-latency mode - All Platforms**

4. **Optimized image quality enhancement algorithms, improving image sharpening and super-resolution effects - All Platforms**

### Bug Fixes

1. Fixed a low-probability crash when creating threads by optimizing SDK thread management.

2. Fixed a low-probability stream playback anomaly.

3. Fixed an occasional crash caused by external filters.
...
1. **Supports for Linux SDK network link binding to specified network interfaces for targeted routing in multi-NIC configurations.**

    - Note: if use please connect Zego Technical Support

2. **Supports Windows ChinaDRM decryption and playback for video streaming and media players.**

    - Note: if use please connect Zego Technical Support

3. **Supports custom encryption for audio and video streams - iOS/Android**

    - Relevate APIs: `setCustomMediaCryptoFactory`

4. **Supports App package name validation. - All Platforms**

    - Note: if use please connect Zego Technical Support

### Improvements & Optimizations

1. **The local device exception callback now supports identifying specific exception types that have recovered from an abnormal state - All Platforms**

2. **Local device exception notification callbacks now support monitoring exception types for low capture volume - All Platforms**

3. **Optimized face detection callback latency - All Platforms**

4. **Optimized memory usage when the media player preloads resources - All Platforms**

5. **Enhanced AGC (Auto Gain Control) performance for singing scenarios - All Platform**

### Bug Fixes

1. Fixed Android a crash issue that occurred during SDK deinitialization when the decoder failed to stop in time.

2. Fixed an issue where RTC service connection failed when local proxy was enabled.

3. Fixed an issue where RTC service connection failed on 32-bit systems when cloud proxy was enabled.

4. Fixed Windows an issue where the preview screen could freeze after system resume from hibernation.

## 3.23.0

### New Features

1. **The onPublisherQualityUpdate callback provides traffic control status information. - All Platforms**

    - Relevate APIs: `onPublisherQualityUpdate`

2. **The setDummyCaptureImagePath interface supports passing in network image addresses. - All Platforms**

    - Relevate APIs: `setDummyCaptureImagePath`

3. **The switchPlayingStream interface supports setting a timeout period. - All Platforms**

    - Relevate APIs: `switchPlayingStream`

4. **The PK Mixer now supports blurred video edges. - All Platforms**

    - Relevate APIs: `startMixerTask`

5. **Supports custom audio preprocessing. - All Platforms**

    - Note: Added the ZegoCustomAudioProcessManager singleton class for registering audio data callbacks.

6. **Web Platform Completion Callback Interface. - Web Platform**

    - Relevate APIs: `onPlayerRecvVideoFirstFrame` `onPlayerVideoSizeChanged` 

### Improvements & Optimizations

1. **Traffic control supports any resolution - All Platforms**

2. **OpenH.264 Encoder Quality Optimization - All Platforms**

3. **Optimization of DNS Resolution for Media Player Resources - All Platforms**

4. **Optimize SDK network connection performance in certain scenarios - All Platforms**

5. **Updated the integrated Express Web SDK to version 3.11.0 - Web Platform**

## 3.22.1

### Bug Fixes

1. Fixed issue where the preview mirror does not take effect.

## 3.22.0

### New Features

1. **Supports mixing the microphone audio source with the screen capture audio source when the primary channel is set as the audio source - iOS/Android**

    - Relevate APIs: `setAudioSource` `startScreenCapture` `updateScreenCaptureConfig` 

2. **Screen sharing provides startup and interruption callbacks - iOS/Android**

    - Relevate APIs: `onMobileScreenCaptureStart` `onMobileScreenCaptureExceptionOccurred` 

3. **Screen sharing supports setting a fixed landscape or portrait orientation - iOS/Android**

    - Relevate APIs: `startScreenCapture` `updateScreenCaptureConfig` 

4. **Screen sharing supports sharing system sounds with the audio package - iOS/Android**

    - Relevate APIs: `startScreenCapture`

5. **Supports starting screen sharing with a specified volume mode - iOS**

    - Relevate APIs: `startScreenCapture` `updateScreenCaptureConfig` 

6. **Added static image capture video source - All Platforms**

    - Relevate APIs: `setVideoSource`

7. **Semi-automatic stream mixing now supports stream alignment - All Platforms**

    - Relevate APIs: `ZegoAutoMixerTask`

8. **The limit on the number of media player instances has been increased to 10 - All Platforms**

9. **The media player supports fragmented file storage for simultaneous downloading and playback - All Platforms**

10. **The stream switching feature supports forced switching mode - All Platforms**

     - Relevate APIs: `switchPlayingStream`

11. **Supports access to the Microframe 264 encoder - All Platforms**

12. **The SDK player supports HLS protocol streaming and adaptive resolution - All Platforms**

    - Relevate APIs: `switchPlayingStream`

13. **Supports simultaneous rendering of video streams to multiple views when pulling video streams - iOS/Android**

    - Relevate APIs: `setPlayingCanvas`

14. **Supports issuing cloud proxy configurations via cloud control, with dynamic implementation - All Platforms**

15. **Added encryption using the National Cryptography Algorithm for streaming push and pull - All Platforms**

16. **Support for WMA encoders - All Platforms**

### Improvements & Optimizations

1. **The curl library in the SDK has been upgraded to version 8.14.1 - All Platforms**

2. **Optimized the issue of excessive memory usage when switching the background type to video with subject segmentation enabled - All Platforms**

3. **Optimized the natural background segmentation effect - All Platforms**

4. **Optimized the AI ​​super-resolution antialiasing effect - All Platforms**

4. **Optimized the low-light enhancement effect - All Platforms**

### Bug Fixes

1. Fixed an issue where repeated camera acquisition when calling startPreview and startPublish asynchronously simultaneously resulted in the camera not being properly released when the stream ended.

2. Fixed an issue where duplicate registration of zego_express_event_handler data in an eventChannel resulted in an error message during callback processing.

3. Fixed an error in the remote device status callback where the onRemoteCameraStateUpdate and onRemoteMicStateUpdate events were not triggered.

4. Fixed the camera permission pop-up issue in audio-only scenes.

5. 
Fixed an issue where the startPlayingStream method failed to start streaming if the view container parameter was not passed.
