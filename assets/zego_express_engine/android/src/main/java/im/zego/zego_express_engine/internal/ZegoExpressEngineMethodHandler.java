...
import im.zego.zegoexpress.callback.IZegoPlayerGetVideoDecoderSupportedCallback;
import im.zego.zegoexpress.callback.IZegoPlayerTakeSnapshotCallback;
import im.zego.zegoexpress.callback.IZegoPublisherGetVideoEncoderSupportedCallback;
...
import im.zego.zegoexpress.callback.IZegoAudioCaptureDeviceEnableCallback;
...
import im.zego.zegoexpress.constants.ZegoStreamAlignmentVolumeControlMode;
...
import im.zego.zegoexpress.constants.ZegoSwitchPlayingStreamType;
...
import im.zego.zegoexpress.constants.ZegoViewUpdateType;
...
import im.zego.zegoexpress.constants.ZegoScreenCaptureOrientation;
import im.zego.zegoexpress.constants.ZegoVolumeType;
import im.zego.zegoexpress.constants.ZegoVideoEncodeEnhanceLevelMode;
import im.zego.zegoexpress.constants.ZegoPublisherTakeSnapshotPosition;
...
import im.zego.zegoexpress.entity.ZegoAudioSourceProcessConfig;
...
import im.zego.zegoexpress.entity.ZegoSwitchPlayingStreamConfig;
...
import im.zego.zegoexpress.entity.ZegoBlurInfo;
...
import im.zego.zegoexpress.entity.ZegoPublisherTakeSnapshotConfig;
import im.zego.zegoexpress.entity.ZegoVideoEncoderEnhancementConfig;
...
public class ZegoExpressEngineMethodHandler {
    ...
    private static final HashMap<Integer, ZegoPictureCapturer> pictureCapturerHashMap = new HashMap<>();

    private static final HashMap<Integer, ZegoCustomAudioSource> customAudioSourceHashMap = new HashMap<>();
    ...
    public static void loginRoom(MethodCall call, final Result result) {
        ...
        if (configMap != null && !configMap.isEmpty()) {
            ...
            if (configMap.containsKey("roomType") && configMap.get("roomType") != null) {
                roomConfig.roomType = ZegoUtils.intValue((Number) configMap.get("roomType"));
            }
        }
    ...
    public static void startPublishingStream(MethodCall call, Result result) {
        ...
        if (configMap != null && !configMap.isEmpty()) {
            ...
            config.streamCensorFlag = ZegoUtils.intValue((Number)configMap.get("streamCensorFlag"));
    ...
    public static void startPreview(MethodCall call, Result result) {
        ...
        if (canvasMap != null && !canvasMap.isEmpty()) {
            ...
            int rotation = ZegoUtils.intValue((Number) canvasMap.get("rotation"));

            Object view;

            if (enablePlatformView) {
                // Render with PlatformView
                PlatformView platformView = ZegoPlatformViewFactory.getInstance().getPlatformView(viewID);
                ...
                // Render with Texture
                ZegoTextureRendererInterface textureRenderer = ZegoTextureRendererController.getInstance().getTextureRenderer((long)viewID);

                if (textureRenderer != null) {
                    view = textureRenderer.getSurface();
                    // When using ZegoSurfaceTextureRenderer, bind channel to textureID for sizeChanged callbacks
                    if (textureRenderer instanceof ZegoSurfaceTextureRenderer) {
                        ZegoTextureRendererController.getInstance().previewTextureInUse.put(channel, textureRenderer.getTextureID());
                    } else {
                        ZegoTextureRendererController.getInstance().previewTextureInUse.remove(channel);
                    }
                } else {
    ...
    @SuppressWarnings("unused")
    public static void takePublishStreamSnapshotByConfig(MethodCall call, final Result result) {
        ZegoPublishChannel channel = ZegoPublishChannel.getZegoPublishChannel(ZegoUtils.intValue((Number) call.argument("channel")));
        HashMap<String, Object> configMap = call.argument("config");
        if (configMap == null || configMap.isEmpty()) {
            result.error("takePublishStreamSnapshotByConfig_Null_Config".toUpperCase(), "[takePublishStreamSnapshotByConfig] Null config", null);
            return;
        }

        int position = ZegoUtils.intValue((Number) configMap.get("position"));
        ZegoPublisherTakeSnapshotConfig config = new ZegoPublisherTakeSnapshotConfig();
        config.position = ZegoPublisherTakeSnapshotPosition.getZegoPublisherTakeSnapshotPosition(position);

        ZegoExpressEngine.getEngine().takePublishStreamSnapshotByConfig(config, new IZegoPublisherTakeSnapshotCallback() {
            @Override
            public void onPublisherTakeSnapshotResult(final int errorCode, final Bitmap image) {
    ...
    @SuppressWarnings("unused")
    public static void getVideoEncoderSupported(MethodCall call, Result result) {
        ZegoVideoCodecID codecID = ZegoVideoCodecID.getZegoVideoCodecID(ZegoUtils.intValue((Number) call.argument("codecID")));
        ZegoVideoCodecBackend codecBackend = ZegoVideoCodecBackend.getZegoVideoCodecBackend(ZegoUtils.intValue((Number) call.argument("codecBackend")));

        ZegoExpressEngine.getEngine().getVideoEncoderSupported(codecID, codecBackend, new IZegoPublisherGetVideoEncoderSupportedCallback() {
            @Override
            public void onPublisherGetVideoEncoderSupportedResult(int support) {
                HashMap<String, Object> resultMap = new HashMap<>();
                resultMap.put("support", support);
                result.success(resultMap);
            }
        });
    }
    ...
    public static void setAudioSource(MethodCall call, Result result) {
        ...
        if (call.argument("config") != null) {
            ...
            ArrayList<Integer> audioSourceIndexList = (ArrayList<Integer>)configMap.get("audioSourceIndexList");
            int[] audioSourceIndexList_ = new int[audioSourceIndexList.size()];
            for (int i = 0; i < audioSourceIndexList.size(); i++) {
                audioSourceIndexList_[i] = audioSourceIndexList.get(i);
            }
            config.audioEffectPlayerIndexList = audioEffectPlayerIndexList_;
            config.mediaPlayerIndexList = mediaPlayerIndexList_;
            config.audioSourceIndexList = audioSourceIndexList_;
            config.enableMixScreenCapture = ZegoUtils.boolValue((Boolean) configMap.get("enableMixScreenCapture"));
        }
    ...
    @SuppressWarnings("unused")
    public static void createCustomAudioSource(MethodCall call, Result result) {

        ZegoCustomAudioSource audioSource = ZegoExpressEngine.getEngine().createCustomAudioSource();

        if (audioSource != null) {
            int index = audioSource.getIndex();
            customAudioSourceHashMap.put(index, audioSource);
    ...
    @SuppressWarnings("unused")
    public static void destroyCustomAudioSource(MethodCall call, Result result) {

        int index = ZegoUtils.intValue((Number) call.argument("index"));
        ZegoCustomAudioSource audioSource = customAudioSourceHashMap.get(index);

        if (audioSource != null) {
            ZegoExpressEngine.getEngine().destroyCustomAudioSource(audioSource);
            customAudioSourceHashMap.remove(index);
        }
    ...
    @SuppressWarnings("unused")
    public static void customAudioSourceStartCapture(MethodCall call, Result result) {

        int index = ZegoUtils.intValue((Number) call.argument("index"));
        ZegoCustomAudioSource audioSource = customAudioSourceHashMap.get(index);

        if (audioSource != null) {
            result.success(audioSource.startCapture());
        } else {
            result.success(-1);
        }
    }
    ...
    @SuppressWarnings("unused")
    public static void customAudioSourceStopCapture(MethodCall call, Result result) {

        int index = ZegoUtils.intValue((Number) call.argument("index"));
        ZegoCustomAudioSource audioSource = customAudioSourceHashMap.get(index);

        if (audioSource != null) {
            result.success(audioSource.stopCapture());
        } else {
            result.success(-1);
        }
    }
    ...
    @SuppressWarnings("unused")
    public static void customAudioSourceEnableAEC(MethodCall call, Result result) {
        ...
        if (audioSource != null) {
            result.success(audioSource.enableAEC(enable));
        } else {
            result.success(-1);
        }
    }
    ...
    @SuppressWarnings("unused")
    public static void customAudioSourceSetVolume(MethodCall call, Result result) {
        ...
        if (audioSource != null) {
            result.success(audioSource.setVolume(volume));
        } else {
            result.success(-1);
        }
    }
    ...
    @SuppressWarnings("unused")
    public static void customAudioSourceMute(MethodCall call, Result result) {
        ...
        if (audioSource != null) {
            result.success(audioSource.mute(mute));
        } else {
            result.success(-1);
        }
    }
    ...
    @SuppressWarnings("unused")
    public static void customAudioSourcePushAudioFrame(MethodCall call, Result result) {
        ...
        if (audioSource != null) {
            int safeDataLength = Math.min(dataLength, data.length);
            ByteBuffer dataByteBuffer = ByteBuffer.allocateDirect(data.length);
            dataByteBuffer = dataByteBuffer.put(data);
            dataByteBuffer.flip();
            result.success(audioSource.pushAudioFrame(dataByteBuffer, safeDataLength, param));
        } else {
            result.success(-1);
        }
    }
    ...
    @SuppressWarnings("unused")
    public static void enableVideoEncoderEnhancement(MethodCall call, Result result) {

        boolean enable = ZegoUtils.boolValue((Boolean) call.argument("enable"));
        ZegoPublishChannel channel = ZegoPublishChannel.getZegoPublishChannel(ZegoUtils.intValue((Number) call.argument("channel")));
        float enhanceLevel = ZegoUtils.floatValue((Number) call.argument("enhanceLevel"));

        ZegoExpressEngine.getEngine().enableVideoEncoderEnhancement(enable, enhanceLevel, channel);

        result.success(null);
    }

    @SuppressWarnings("unused")
    public static void enableVideoEncoderEnhancementWithConfig(MethodCall call, Result result) {

        boolean enable = ZegoUtils.boolValue((Boolean) call.argument("enable"));
        ZegoPublishChannel channel = ZegoPublishChannel.getZegoPublishChannel(ZegoUtils.intValue((Number) call.argument("channel")));

        HashMap<String, Object> configMap = call.argument("config");
        ZegoVideoEncoderEnhancementConfig config = new ZegoVideoEncoderEnhancementConfig();
        config.enhanceLevelMode = ZegoVideoEncodeEnhanceLevelMode.getZegoVideoEncodeEnhanceLevelMode(ZegoUtils.intValue((Number) configMap.get("enhanceLevelMode")));
        config.enhanceLevel = ZegoUtils.floatValue((Number) configMap.get("enhanceLevel")) ;
        
        ZegoExpressEngine.getEngine().enableVideoEncoderEnhancementWithConfig(enable, config, channel);
    ...
    public static void startPlayingStream(MethodCall call, Result result) {
        ...
        if (canvasMap != null && !canvasMap.isEmpty()) {
            ...
            boolean alphaBlend = ZegoUtils.boolValue((Boolean) canvasMap.get("alphaBlend"));
            int rotation = ZegoUtils.intValue((Number) canvasMap.get("rotation"));
            boolean mirror = ZegoUtils.boolValue((Boolean) canvasMap.get("mirror"));
            ...
            if (enablePlatformView) {
                ...
                if (textureRenderer != null) {
                    view = textureRenderer.getSurface();
                    // When using ZegoSurfaceTextureRenderer, bind streamID to textureID for sizeChanged callbacks
                    if (textureRenderer instanceof ZegoSurfaceTextureRenderer) {
                        ZegoTextureRendererController.getInstance().playerTextureInUse.put(streamID, textureRenderer.getTextureID());
                    } else {
                        ZegoTextureRendererController.getInstance().playerTextureInUse.remove(streamID);
                    }
                } else {
    ...
    public static void switchPlayingStream(MethodCall call, Result result) {
        ...
        if (playerConfigMap != null && !playerConfigMap.isEmpty()) {
            ...
            HashMap<String, Object> switchStreamConfigMap = (HashMap<String, Object>) playerConfigMap.get("switchStreamConfig");
            if (switchStreamConfigMap != null && !switchStreamConfigMap.isEmpty()) {
                ZegoSwitchPlayingStreamConfig switchStreamConfig = new ZegoSwitchPlayingStreamConfig();
                switchStreamConfig.switchType = ZegoSwitchPlayingStreamType.getZegoSwitchPlayingStreamType(ZegoUtils.intValue((Number) switchStreamConfigMap.get("switchType")));
                switchStreamConfig.switchTimeout = ZegoUtils.intValue((Number) switchStreamConfigMap.get("switchTimeout"));
                playerConfig.switchStreamConfig = switchStreamConfig;
            }
        }
    ...
    public static void switchPlayingResource(MethodCall call, Result result) {

        String streamID = call.argument("streamID");
        int resourceTypeIndex = ZegoUtils.intValue((Number) call.argument("resourceType"));
        ZegoResourceType resourceType = ZegoResourceType.getZegoResourceType(resourceTypeIndex);

        ZegoExpressEngine.getEngine().switchPlayingResource(streamID, resourceType);
    ...
    @SuppressWarnings("unused")
    public static void getVideoDecoderSupported(MethodCall call, Result result) {
        ZegoVideoCodecID codecID = ZegoVideoCodecID.getZegoVideoCodecID(ZegoUtils.intValue((Number) call.argument("codecID")));
        ZegoVideoCodecBackend codecBackend = ZegoVideoCodecBackend.getZegoVideoCodecBackend(ZegoUtils.intValue((Number) call.argument("codecBackend")));

        ZegoExpressEngine.getEngine().getVideoDecoderSupported(codecID, codecBackend, new IZegoPlayerGetVideoDecoderSupportedCallback() {
            @Override
            public void onPlayerGetVideoDecoderSupportedResult(int support) {
                HashMap<String, Object> resultMap = new HashMap<>();
                resultMap.put("support", support);
                result.success(resultMap);
            }
    ...
    @SuppressWarnings("unused")
    public static void setPlayingCanvas(MethodCall call, Result result) {

        String streamID = call.argument("streamID");
        // ZegoStreamAlignmentMode mode = ZegoStreamAlignmentMode.getZegoStreamAlignmentMode(ZegoUtils.intValue((Number) call.argument("mode")));
        ZegoViewUpdateType updateType = ZegoViewUpdateType.getZegoViewUpdateType(ZegoUtils.intValue((Number) call.argument("updateType")));
        // Handle ZegoCanvas
        ...
        if (canvasMap != null && !canvasMap.isEmpty()) {
            ...
            int rotation = ZegoUtils.intValue((Number) canvasMap.get("rotation"));
            boolean mirror = ZegoUtils.boolValue((Boolean) canvasMap.get("mirror"));
            String viewContext = (String) canvasMap.get("viewContext");
            ...
            if (enablePlatformView) {
                ...
                if (platformView != null) {
                    ...
                    // Need to invoke dart `createPlatformView` method in advance to create PlatformView and get viewID (PlatformViewID)
                    String errorMessage = String.format(Locale.ENGLISH, "The PlatformView for viewID:%d cannot be found, developer should call `createPlatformView` first and get the viewID", viewID);
                    ZegoLog.error("[setPlayingCanvas] %s", errorMessage);
                    result.error("setPlayingCanvas_No_PlatformView".toUpperCase(), errorMessage, null);
                    return;
                }
                ...
                if (textureRenderer != null) {
                    ...
                    String errorMessage = String.format(Locale.ENGLISH, "The TextureRenderer for textureID:%d cannot be found, developer should call `createCanvasView` first and get the textureID", viewID);
                    ZegoLog.error("[setPlayingCanvas] %s", errorMessage);
                    result.error("setPlayingCanvas_No_TextureRenderer".toUpperCase(), errorMessage, null);
                    return;
                }
            }
            ...
            if (view != null) {
                ...
                canvas.rotation = rotation;
                canvas.mirror = mirror;
                canvas.viewContext = viewContext;
            }
        ...
        int errorCode = ZegoExpressEngine.getEngine().setPlayingCanvas(streamID, updateType, canvas);
    ...
    public static void startMixerTask(MethodCall call, final Result result) {
        ...
        if (inputListMap != null && !inputListMap.isEmpty()) {
            ...
            for (HashMap<String, Object> inputMap: inputListMap) {
                ...
                if(inputMap.containsKey("blurInfo") && inputMap.get("blurInfo") != null)
                {
                    HashMap<String, Object> blurInfoMap = (HashMap<String, Object>) inputMap.get("blurInfo");
                    ZegoBlurInfo blurInfo = new ZegoBlurInfo();
                    blurInfo.topPadding = ZegoUtils.intValue((Number) blurInfoMap.get("topPadding"));
                    blurInfo.leftPadding = ZegoUtils.intValue((Number) blurInfoMap.get("leftPadding"));
                    blurInfo.bottomPadding = ZegoUtils.intValue((Number) blurInfoMap.get("bottomPadding"));
                    blurInfo.rightPadding = ZegoUtils.intValue((Number) blurInfoMap.get("rightPadding"));
                    inputObject.blurInfo = blurInfo;
                }
    ...
    public static void startAutoMixerTask(MethodCall call, final Result result) {
        ...
        taskObject.streamAlignmentVolumeControlMode = ZegoStreamAlignmentVolumeControlMode.getZegoStreamAlignmentVolumeControlMode(ZegoUtils.intValue((Number)call.argument("streamAlignmentVolumeControlMode")));
        taskObject.streamAlignmentBaselineStreamID = call.argument("streamAlignmentBaselineStreamID");
    ...
    public static void enableAudioCaptureDevice(MethodCall call, final Result result) {
        ...
        ZegoExpressEngine.getEngine().enableAudioCaptureDeviceAsync(enable, new IZegoAudioCaptureDeviceEnableCallback() {
            @Override
            public void onAudioCaptureDeviceEnableResult(int errorCode) {
                result.success(null);
            }
        });
    }
    ...
    @SuppressWarnings("unused")
    public static void enableBeforeAudioPrepAudioData(MethodCall call, Result result) {
        ...
        ZegoExpressEngine.getEngine().enableBeforeAudioPrepAudioData(enable, param);
    ...
    @SuppressWarnings("unused")
    public static void sendCustomAudioCaptureOpusData(MethodCall call, Result result) {
        ...
        dataByteBuffer.flip();
        int dataLength = ZegoUtils.intValue((Number) call.argument("dataLength"));
        long referenceTimeMillisecond = ZegoUtils.longValue((Number) call.argument("referenceTimeMillisecond"));
        int samples = ZegoUtils.intValue((Number) call.argument("samples"));
        HashMap<String, Object> paramMap = call.argument("param");
        ...
        ZegoExpressEngine.getEngine().sendCustomAudioCaptureOpusData(dataByteBuffer, dataLength, referenceTimeMillisecond, samples, param, channel);
    ...
    public static void destroyMediaPlayer(MethodCall call, Result result) {
        ...
        if (!enablePlatformView) {
            ...
            ZegoTextureRendererController.getInstance().mediaPlayerTextureInUse.remove(index);
        }
    ...
    public static void mediaPlayerSetPlayerCanvas(MethodCall call, Result result) {
        ...
        if (canvasMap != null && !canvasMap.isEmpty()) {
            ...
            if (enablePlatformView) {
                ...
                if (textureRenderer != null) {
                    view = textureRenderer.getSurface();
                    // When using ZegoSurfaceTextureRenderer, bind mediaPlayer index to textureID for sizeChanged callbacks
                    if (textureRenderer instanceof ZegoSurfaceTextureRenderer) {
                        ZegoTextureRendererController.getInstance().mediaPlayerTextureInUse.put(index, textureRenderer.getTextureID());
                    } else {
                        ZegoTextureRendererController.getInstance().mediaPlayerTextureInUse.remove(index);
                    }
                } else {
    ...
    public static void startDumpData(MethodCall call, Result result) {
        ...
        // config.dataType
        HashMap<String, Object> configMap = call.argument("config");
        int dataType = ZegoUtils.intValue((Number) configMap.get("dataType"));
        config.dataType = ZegoDumpDataType.getZegoDumpDataType(dataType);
    ...
    public static void startCaptureScreenCaptureSource(MethodCall call, Result result) {
        ...
        if (configMap != null) {
            ...
            config.orientation = ZegoScreenCaptureOrientation.getZegoScreenCaptureOrientation(ZegoUtils.intValue((Number) configMap.get("orientation")));
            config.applicationVolume = ZegoUtils.intValue((Number) configMap.get("applicationVolume"));
    ...
    /* Picture Capturer */
    @SuppressWarnings("unused")
    public static void createPictureCapturer(MethodCall call, Result result) {
        ZegoPictureCapturer pictureCapturer = ZegoExpressEngine.getEngine().createPictureCapturer();
        if (pictureCapturer != null) {
            int index = pictureCapturer.getIndex();
            pictureCapturerHashMap.put(index, pictureCapturer);
            result.success(index);
        } else {
            result.success(-1);
        }
    ...
    @SuppressWarnings("unused")
    public static void destroyPictureCapturer(MethodCall call, Result result) {
        int index = ZegoUtils.intValue((Number) call.argument("index"));
        ZegoPictureCapturer pictureCapturer = pictureCapturerHashMap.get(index);
        if (pictureCapturer != null) {
            ZegoExpressEngine.getEngine().destroyPictureCapturer(pictureCapturer);
        }
        pictureCapturerHashMap.remove(index);
        result.success(null);
    }

    @SuppressWarnings("unused")
    public static void setPathPictureCapturer(MethodCall call, Result result) {
        int index = ZegoUtils.intValue((Number) call.argument("index"));
        String path = call.argument("path");
        if (path != null && path.startsWith("flutter-asset://")) {
            String processedURL = path.replace("flutter-asset://", "asset:flutter_assets/");
            ZegoLog.log("[setDummyCaptureImagePath] Flutter asset prefix detected, origin URL: '%s', processed URL: '%s'", path, processedURL);
            path = processedURL;
        }
        ZegoPictureCapturer pictureCapturer = pictureCapturerHashMap.get(index);
        if (pictureCapturer != null) {
    ...
    public static void createTextureRenderer(MethodCall call, Result result) {
        ...
        Long textureID = ZegoTextureRendererController.getInstance().createTextureRenderer(textureRegistry.createSurfaceTexture(), width, height, args);
