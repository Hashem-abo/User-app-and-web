...
struct _ZegoTexture {
    ...
    std::weak_ptr<VideoOutput> video_output;
};
...
gboolean zego_texture_copy_pixels(FlPixelBufferTexture *texture, const uint8_t **buffer,
    ...
    std::shared_ptr<VideoOutput> video_output = self->video_output.lock();
    if (!video_output) {
        g_set_error(error, g_quark_from_static_string("zego-texture"), 0,
                    "texture has no frame available");
        return FALSE;
    }

    const std::lock_guard<std::mutex> lock(video_output->mutex);
    if (video_output->buffer.empty()) {
        g_set_error(error, g_quark_from_static_string("zego-texture"), 0,
                    "texture has no frame available");
        return FALSE;
    }

    *buffer = video_output->buffer.data();
