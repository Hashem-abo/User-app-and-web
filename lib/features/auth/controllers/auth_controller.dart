...
class AuthController extends GetxController implements GetxService {
  ...
  Future<bool> clearSharedData({bool removeToken = true}) async {
    if(!ResponsiveHelper.isDesktop(Get.context)){
      Get.find<SplashController>().setModule((Get.find<SplashController>().moduleList != null && Get.find<SplashController>().moduleList!.isNotEmpty)
          ? Get.find<SplashController>().moduleList![0] : null);
    }
  ...
  Future<bool> saveProductRefCode(String code) async {
    return await authServiceInterface.saveProductRefCode(code);
  }

  String getProductRefCode() {
    return authServiceInterface.getProductRefCode();
  }

  Future<bool> clearProductRefCode() async {
    return await authServiceInterface.clearProductRefCode();
  }
