...
class ItemThatYouLoveView extends StatefulWidget {
  ...
  final String? title;
  final double? height;
  const ItemThatYouLoveView({super.key, required this.forShop, this.isFood = false, this.isShop = false, this.title, this.height});
...
class _ItemThatYouLoveViewState extends State<ItemThatYouLoveView> {
  ...
  void initState() {
    ...
    if (itemController.recommendedItemList == null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        itemController.getRecommendedItemList(false, 'all', true);
      });
    }
    if(itemController.recommendedItemList != null){
      _currentPage = itemController.recommendedItemList!.length > 1 ? 1: 0;
    }
  ...
  void _startTimer() {
    _timer = Timer.periodic(const Duration(seconds: 3), (timer) {
      if(Get.find<ItemController>().recommendedItemList != null && Get.find<ItemController>().recommendedItemList!.isNotEmpty) {
        if (_currentPage < Get.find<ItemController>().recommendedItemList!.length - 1) {
          _currentPage++;
        } else {
          _currentPage = 0;
        }
      ...
      return recommendItems != null ? recommendItems.isNotEmpty ? Column(children: [
        ...
        Padding(
          ...
          child: Align(
            ...
            child: Text(widget.title ?? 'item_that_you_love'.tr, style: robotoBold.copyWith(fontSize: Dimensions.fontSizeLarge)),
          ),
        ),
        ...
        widget.forShop ? Padding(
          ...
          child: Stack(
            children: [
              SizedBox(
                height: widget.height ?? 380, width: Get.width,
                child: Swiper(
  ...
  Widget carouselCard(int index, Item item) {
    return Padding(
      ...
      child: Hero(
        tag: "item_love_image_${item.id}",
        child: ItemCard(
          item: item, 
          index: index,
          isPopularItem: widget.isShop ? false : true,
          isPopularItemCart: true,
          isFood: widget.isFood, 
          isShop: widget.isShop,
        ),
...
class _ItemThatYouLoveShimmerViewState extends State<ItemThatYouLoveShimmerView> {
  ...
  void initState() {
    super.initState();
    pageController = PageController(initialPage: _currentPage, viewportFraction: 0.5);
  }
                ...
                builder: (context, child) {
                  ...
                  return Transform.scale(
                    ...
                    child: Padding(
                      padding: const EdgeInsets.all(Dimensions.paddingSizeExtraSmall),
                      child: Hero(
                        tag: "shimmer_item_love_image$index",
