...
class StoreRepository implements StoreRepositoryInterface {
  ...
  Future<RecommendedItemModel?> _getStoreRecommendedItemList(int? storeId) async {
    ...
    Response response = await apiClient.getData('${AppConstants.storeRecommendedItemUri}?store_id=$storeId&offset=1&limit=12');
    if(response.statusCode == 200){
  ...
  Future<CartSuggestItemModel?> getCartStoreSuggestedItemList(int? storeId, String languageCode, ModuleModel? module, int? cacheModuleId, int? moduleId) async {
    ...
    Response response = await apiClient.getData('${AppConstants.cartStoreSuggestedItemsUri}?recommended=1&store_id=$storeId&offset=1&limit=12', headers: header);
    if(response.statusCode == 200){
