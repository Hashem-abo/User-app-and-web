...
class CartService implements CartServiceInterface {
  ...
  Future<bool> clearCartOnline({int? moduleId}) async {
    return await cartRepositoryInterface.delete(null, isRemoveAll: true, moduleId: moduleId);
  }
  ...
  List<CartModel> formatOnlineCartToLocalCart({required List<OnlineCartModel> onlineCartModel}) {
    ...
    for (OnlineCartModel cart in onlineCartModel) {
      ...
      cartList.add(
        CartModel(
          id: cart.id, price: price, discountedPrice: discountedPrice, variation: cart.productVariation?? [], foodVariations: selectedFoodVariations, discountAmount: discountAmount, quantity: quantity,
          addOnIds: addOnIdList, addOns: addOnsList, isCampaign: false, stock: stock, item: cart.item, quantityLimit: quantityLimit, note: cart.note,
        ),
      );
    }
  ...
  List<int> getModuleWithCartList() {
    return cartRepositoryInterface.getModuleWithCartList();
  }

  @override
  Map<int, int> getModuleCartCounts() {
    return cartRepositoryInterface.getModuleCartCounts();
  }
