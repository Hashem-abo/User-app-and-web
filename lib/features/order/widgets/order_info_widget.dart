...
import 'package:suliman/common/widgets/custom_card.dart';
...
import 'package:suliman/helper/call_helper.dart';
...
import 'package:suliman/features/chat/widgets/image_dialog_widget.dart';
import 'package:suliman/features/payment/widgets/offline_info_edit_dialog_widget.dart';
import 'package:suliman/features/order/widgets/order_banner_view_widget.dart';
import 'package:suliman/features/order/widgets/order_item_widget.dart';
import 'package:suliman/features/parcel/widgets/details_widget.dart';
import 'package:suliman/features/review/widgets/review_dialog_widget.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(children: [
      ...
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        CustomCard(
          isBorder: false, borderRadius: Dimensions.radiusDefault,
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.start, children: [
            ...
            order.orderStatus != 'canceled' && (Get.find<SplashController>().configModel?.orderDeliveryVerification ?? false) ? Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              InkWell(
                ...
                child: Row(
                  children: [
                    Text(order.otp ?? '', style: robotoBold.copyWith(color: Theme.of(context).primaryColor)),
        ...
        (order.deliveryMan != null && isDesktop) ? Text(order.store?.id == 15 ? 'store_details'.tr : 'delivery_man_details'.tr, style: robotoMedium) : const SizedBox(),
        ...
        order.deliveryMan != null ? CustomCard(
          ...
          padding: EdgeInsets.all(Dimensions.paddingSizeDefault),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(order.store?.id == 15 ? 'store_details'.tr : 'delivery_man_details'.tr, style: robotoSemiBold),
        ...
        (!parcel && isDesktop && order.store?.moduleId != 1) ? Text('delivery_details'.tr, style: robotoMedium) :  const SizedBox(),
        (!parcel && isDesktop && order.store?.moduleId != 1) ? const SizedBox(height: Dimensions.paddingSizeSmall) : const SizedBox(),

        (!parcel && order.store != null && order.store?.moduleId != 1) ? CustomCard(
          borderRadius: isDesktop ? Dimensions.radiusDefault : 0, isBorder: false,
          padding: EdgeInsets.all(Dimensions.paddingSizeDefault),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('delivery_details'.tr, style: robotoSemiBold),
              (order.orderStatus == 'picked_up' && order.deliveryMan != null) ? TextButton.icon(
                ...
                onPressed: () async {
                  timerCancel();
                  await Get.toNamed(RouteHelper.getOrderTrackingMapRoute(order.id, order.deliveryAddress?.contactPersonNumber))?.whenComplete(() => startApiCall());
                },
                icon: const Icon(Icons.directions, size: 18),
                label: Text('track_on_map'.tr, style: robotoBold.copyWith(fontSize: Dimensions.fontSizeSmall)),
              ) : const SizedBox(),
            ]),
            ...
            Row(children: [
              Expanded(
                child: Row(children: [
                  ...
                  const SizedBox(width: Dimensions.paddingSizeSmall),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('from_store'.tr, style: robotoMedium.copyWith(fontSize: Dimensions.fontSizeSmall)),
              ...
              Padding(
                ...
                child: Icon(Icons.arrow_forward, size: 16, color: Theme.of(context).disabledColor),
              ),
              Expanded(
                child: Row(children: [
                  ...
                  const SizedBox(width: Dimensions.paddingSizeSmall),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('delivery_to'.tr, style: robotoMedium.copyWith(fontSize: Dimensions.fontSizeSmall)),
                    const SizedBox(height: 2),
                    Text(
