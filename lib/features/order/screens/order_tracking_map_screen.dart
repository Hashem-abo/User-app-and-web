...
import 'package:suliman/features/location/widgets/permission_dialog_widget.dart';
...
import 'package:suliman/helper/marker_helper.dart';
import 'package:suliman/helper/pusher_helper.dart';
...
import 'package:suliman/features/order/widgets/track_details_view_widget.dart';
import 'package:suliman/features/order/widgets/tracking_stepper_widget.dart';
...
class OrderTrackingMapScreen extends StatefulWidget {
  ...
  const OrderTrackingMapScreen({super.key, required this.orderID, this.contactNumber});

  @override
  OrderTrackingMapScreenState createState() => OrderTrackingMapScreenState();
}

class OrderTrackingMapScreenState extends State<OrderTrackingMapScreen> with WidgetsBindingObserver {
  ...
  void _timerTrackOrder(){
    if(Get.find<OrderController>().trackModel?.orderStatus != 'delivered' && Get.find<OrderController>().trackModel?.orderStatus != 'failed' && Get.find<OrderController>().trackModel?.orderStatus != 'canceled') {
      ...
      _timer = Timer.periodic(const Duration(seconds: 10), (timer) {
        if(Get.currentRoute.contains(RouteHelper.orderDetails) || Get.currentRoute.contains(RouteHelper.orderTracking) || Get.currentRoute.contains(RouteHelper.orderTrackingMap)){
          Get.find<OrderController>().timerTrackOrder(widget.orderID.toString(), contactNumber: widget.contactNumber);
