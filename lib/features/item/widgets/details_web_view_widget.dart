...
import 'package:suliman/common/widgets/cart_snackbar.dart';
...
import 'package:suliman/features/item/widgets/item_title_view_widget.dart';
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        GetBuilder<CartController>(builder: (cartController) {
                          bool isExist = itemController.cartIndex != -1 && itemController.cartIndex < cartController.cartList.length;
                          return Row(children: [
                            Text('${'total_amount'.tr}:', style: robotoBold),
                            const SizedBox(width: Dimensions.paddingSizeExtraSmall),

                            Text(PriceConverter.convertPrice(isExist
                                ? _getItemDetailsDiscountPrice(cart: cartController.cartList[itemController.cartIndex])
                        ...
                        Row(children: [
                          ...
                          GetBuilder<CartController>(builder: (cartController) {
                            ...
                            return Row(children: [
                              QuantityButton(
                                isIncrement: false, quantity: isExist ? cartController.cartList[itemController.cartIndex].quantity : itemController.quantity,
                                stock: stock, isExistInCart : isExist, cartIndex: itemController.cartIndex,
                                quantityLimit : isExist ? cartController.cartList[itemController.cartIndex].quantityLimit : itemController.item!.quantityLimit,
                                cartController: cartController,
                              ),
                              const SizedBox(width: 30),

                              Text(
                                isExist ? cartController.cartList[itemController.cartIndex].quantity.toString() : itemController.quantity.toString(),
                                style: robotoBold.copyWith(fontSize: Dimensions.fontSizeExtraLarge),
                              ),
                              const SizedBox(width: 30),

                              QuantityButton(
                                isIncrement: true, quantity: isExist ? cartController.cartList[itemController.cartIndex].quantity : itemController.quantity,
                                stock: stock, cartIndex: itemController.cartIndex, isExistInCart: isExist,
                                quantityLimit : isExist ? cartController.cartList[itemController.cartIndex].quantityLimit : itemController.item!.quantityLimit,
                                cartController: cartController,
                              ),
                                ...
                                onPressed: (ItemHelper.isItemEntirelyOutOfStock(itemController.item) || (itemController.item?.quantityLimit != null && itemController.item!.quantityLimit == 0)) ? null : () async {
                                  if(itemController.item!.availableDateStarts != null) {
                                     ...
                                     Get.dialog(ConfirmationDialog(
                                       ...
                                       onYesPressed: () {
                                        ...
                                        cartController.clearCartOnline().then((success) async {
                                          if(success) {
                                            cartController.addToCartOnline(cart!, cartModel!);
                                    ...
                                    if(itemController.cartIndex == -1) {
                                      ...
                                      cartController.updateCartOnline(cart!, cartModel!);
                                      showCartSnackBar();
                                    }
                                  }
                                },
