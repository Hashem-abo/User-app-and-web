...
import 'package:suliman/features/checkout/widgets/checkout_screen_shimmer_view.dart';
import 'package:suliman/features/checkout/widgets/payment_method_bottom_sheet.dart';
import 'package:get/get.dart';
import 'package:suliman/features/checkout/widgets/bottom_section.dart';
import 'package:suliman/features/checkout/widgets/top_section.dart';
...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> initCall() async {
    ...
    if (isLoggedIn) {
      Get.find<ProfileController>().getUserInfo().then((_) {
        final phone = Get.find<ProfileController>().userInfoModel?.phone;
        if (phone == null ||
            phone.isEmpty ||
            phone == 'null' ||
            phone == '0000000000') {
          final prefAddr = AddressHelper.getUserAddressFromSharedPref();
          if (prefAddr?.contactPersonNumber != null &&
              prefAddr!.contactPersonNumber!.isNotEmpty &&
              prefAddr.contactPersonNumber != 'null' &&
              prefAddr.contactPersonNumber != '0000000000') {
            if (guestContactPersonNumberController.text.isEmpty) {
              guestContactPersonNumberController.text =
                  prefAddr.contactPersonNumber!;
            }
          }
        }
      });
    ...
    if (widget.storeId == null || widget.cartList != null) {
      ...
      if (_cartList != null &&
          ...
          widget.storeId == null) {
        await Get.find<CheckoutController>().initCheckoutData(null);
      }
    }
    ...
    if (AuthHelper.isGuestLoggedIn()) {
      guestEmailController.text = AuthHelper.getGuestEmail();
    }
  }
          ...
          ? GetBuilder<ProController>(builder: (proController) {
              return GetBuilder<CheckoutController>(
                ...
                return GetBuilder<AddressController>(
                  ...
                  AddressModel? userAddrForZone =
                      AddressHelper.getUserAddressFromSharedPref();
                  bool isOutZoneStore = false;
                  if (checkoutController.store != null &&
                      userAddrForZone != null) {
                    bool isAllZones = (userAddrForZone.zoneId == 0) ||
                        (userAddrForZone.zoneIds != null &&
                            userAddrForZone.zoneIds!.contains(0));
                    if (!isAllZones) {
                      if (userAddrForZone.zoneIds != null &&
                          userAddrForZone.zoneIds!.isNotEmpty) {
                        isOutZoneStore = !userAddrForZone.zoneIds!
                            .contains(checkoutController.store!.zoneId);
                      } else if (userAddrForZone.zoneId != null &&
                          checkoutController.store!.zoneId != null) {
                        isOutZoneStore = userAddrForZone.zoneId !=
                            checkoutController.store!.zoneId;
                      }
                    }
                  }
                  if (isOutZoneStore &&
                      checkoutController.orderType != 'pickup_center') {
                    WidgetsBinding.instance.addPostFrameCallback((_) {
                      checkoutController.setOrderType('pickup_center',
                          notify: true);
                    });
                  }
                  ...
                  return GetBuilder<CouponController>(
                    ...
                    double couponDiscount =
                        PriceConverter.toFixed(couponController.discount ?? 0);
                    ...
                    Future.delayed(const Duration(milliseconds: 50), () {
                      ...
                      if (checkoutController.isFirstTime ||
                        ...
                        if (widget.storeId == null ||
                            (_cartList != null && _cartList!.isNotEmpty)) {
                          for (int index = 0;
                              index < (_cartList?.length ?? 0);
                              index++) {
                            ...
                            bool isNewVar = (Get.find<SplashController>()
                            ...
                            carts.add(OnlineCart(
                              cartId: cart.id,
                              itemId: cart.item!.id,
                              itemCampaignId: (cart.isCampaign ?? false)
                                  ? cart.item!.id
                                  : null,
                              ...
                              model: 'Item',
                              itemType: (cart.isCampaign ?? false)
                        ...
                        PlaceOrderBodyModel placeOrderBody =
                          ...
                          storeDistances: checkoutController.storeDistances
                              .map((key, value) =>
                          ...
                          paymentMethod: checkoutController
                              ...
                              : checkoutController.paymentMethodIndex == 1
                                  ...
                                  : checkoutController.paymentMethodIndex == 2
                                      ? 'digital_payment'
                                      : 'offline_payment',
                          couponCode: (currentCouponDiscount > 0 ||
                                  (couponController.coupon != null &&
                                      couponController.freeDelivery))
                              ? couponController.coupon?.code
                              : null,
                          ...
                          isPrescriptionOrder: (widget.storeId != null &&
                              (_cartList == null || _cartList!.isEmpty)),
                          bringChangeAmount:
                              checkoutController.paymentMethodIndex == 0 &&
                          ...
                          digitalPaymentGateway:
                              checkoutController.paymentMethodIndex == 2
                                  ...
                                  : null,
                          purchaseCode: checkoutController.paymentMethodIndex ==
                                      2 &&
                                  (checkoutController.digitalPaymentName ==
                                          'easy_wallet' ||
                                      checkoutController.digitalPaymentName ==
                                          'floosak')
                              ? checkoutController.purchaseCodeController.text
                              : null,
                          productReferrerId: Get.find<AuthController>()
                                  .getProductRefCode()
                                  .isNotEmpty
                              ? Get.find<AuthController>().getProductRefCode()
                              : null,
                    ...
                    double additionalCharge = (Get.find<SplashController>()
                        ...
                        ? (Get.find<SplashController>()
                            ...
                            0)
                        : 0;
                    int addrIndex = (checkoutController.addressIndex != null &&
                    ...
                    double deliveryCharge = _calculateDeliveryCharge(
                      ...
                      distance: checkoutController.distance,
                      extraCharge: checkoutController.extraCharge,
                      orderType: checkoutController.orderType ?? 'delivery',
                    ...
                    if (checkoutController.orderType != 'take_away' &&
                      ...
                      if (fbsFee != null &&
                        ...
                        _deliveryChargeForView = deliveryCharge == -1
                            ? 'calculating'.tr
                            : deliveryCharge == 0
                                ? 'free'.tr
                    ...
                    final ProActiveBenefit? proBenefit =
                        Get.find<ProController>().activeOfferModel?.benefit;
                    final bool isCampaign = _cartList != null &&
                        _cartList!.isNotEmpty &&
                        (_cartList!.first?.isCampaign ?? false);
                    final bool isPro = !isCampaign &&
                        (Get.find<ProfileController>()
                                .userInfoModel
                                ?.proStatus ??
                            false);
                    double proDiscount = isPro
                        ? _calculateProDiscount(
                            cartList: _cartList,
                            benefit: proBenefit,
                            fallbackSubTotal: subTotal,
                            fallbackDiscount: discount,
                            fallbackCouponDiscount: couponDiscount,
                          )
                        : 0;
                    double proDeliveryDiscount = (isPro &&
                            checkoutController.orderType != 'take_away' &&
                            checkoutController.orderType != 'dine_in')
                        ? _calculateProDeliveryDiscount(
                            subTotal - discount - couponDiscount,
                            deliveryCharge,
                            proBenefit)
                        : 0;
                    // Pro fully covers the delivery fee (full-free or 100% off) -> saver charge options are pointless.
                    final bool proFreeDelivery = proDeliveryDiscount > 0 &&
                        (proBenefit?.offerType == ProOfferType.fullFree ||
                            (proBenefit?.chargeDiscountPercentage ?? 0) >= 100);

                    // Saver delivery option fee adjustment (express +charge / slightly_delay -charge); 0 for the standard option.
                    final double saverDeliveryAdjustment = (checkoutController
                                    .orderType ==
                                'delivery' &&
                            checkoutController.isInstantDelivery)
                        ? checkoutController.getSaverDeliveryChargeAdjustment(
                            deliveryOption:
                                checkoutController.selectedSaverDeliveryOption)
                        : 0;
                    ...
                    double total = _calculateTotal(
                      ...
                      tax: checkoutController.orderTax ?? 0,
                      orderType: checkoutController.orderType ?? 'delivery',
                    ...
                    total = total -
                        ...
                        proDiscount -
                        proDeliveryDiscount +
                        saverDeliveryAdjustment;

                    if (widget.storeId != null &&
                        !AuthHelper.isGuestLoggedIn() &&
                        checkoutController.isFirstTimeCodActive) {
                      WidgetsBinding.instance.addPostFrameCallback((_) {
                        if (checkoutController.isFirstTimeCodActive) {
                          checkoutController.setPaymentMethod(0,
                              isUpdate: false);
                                                    ...
                                                    child: Column(
                                                      ...
                                                      children: [
                                                        if (checkoutController
                                                            .isFbsFulfilled)
                                                          Text(
                                                            'fbs_consolidated_delivery_title'
                                                                .tr,
                                                        ...
                                                        Text(
                                                          checkoutController
                                                                  .isFbsFulfilled
                                                              ? 'fbs_consolidated_delivery_notice'
                                                                  .tr
                                                              : "note_more_than_one_order"
                                                                  .tr,
                                                          style: robotoRegular
                                                              .copyWith(
                                                            fontSize: Dimensions
          ...
          children: [
            ...
            CustomButton(
                isLoading: checkoutController.isLoading ||
                    checkoutController.isSubmittingOrder,
                buttonText:
                    '${'confirm_order'.tr} - ${PriceConverter.convertPrice(total)}',
                    ...
                    ? () async {
                        ...
                        if (checkoutController.timeSlots == null ||
                          ...
                          DateTime date = DateTime.now().add(Duration(
                              days: checkoutController.selectedDateSlot));
                        ...
                        bool hasProfilePhone = !isGuestLogIn &&
                            profilePhone != null &&
                        ...
                        if (isGuestLogIn &&
                        ...
                        } else if ((isGuestLogIn || !hasProfilePhone) &&
                            (guestContactPersonNumberController.text.trim().isEmpty ||
                                guestContactPersonNumberController.text.trim().length <
                                    6)) {
                          showCustomSnackBar(
                          ...
                          showCustomSnackBar('no_payment_method_is_enabled'.tr);
                            ...
                            (checkoutController.digitalPaymentName == 'easy_wallet' ||
                                checkoutController.digitalPaymentName ==
                                    'floosak') &&
                            checkoutController.purchaseCodeController.text
                                .trim()
                                .isEmpty) {
                          showCustomSnackBar(
                              'Please enter your Purchase Code'.tr);
                          ...
                          showCustomSnackBar(
                              'one_or_more_products_are_not_available_for_this_selected_time'
                                  .tr);
                        } else if (checkoutController.orderType == 'pickup_center' &&
                            checkoutController.selectedPickupCenter == null) {
                          ...
                          showCustomSnackBar(
                            ...
                            (address.isEmpty ||
                                !address[checkoutController.addressIndex!]
                                    .zoneIds!
                                    .contains(checkoutController.store!.zoneId))) {
                          showCustomSnackBar(address.isEmpty
                              ? 'please_setup_your_delivery_address_first'.tr
                              : 'delivery_address_is_not_valid_for_the_selected_zone'
                                  .tr);
                        } else {
                          int addrIndex =
                          ...
                          String enteredPhoneText =
                              guestContactPersonNumberController.text.trim();
                          String enteredFullPhone = enteredPhoneText
                                  .startsWith('+')
                              ? enteredPhoneText
                          ...
                          if (isGuestLogIn) {
                            ...
                            finalAddress = AddressModel(
                              ...
                              contactPersonName:
                                  guestContactPersonNameController
                                      ...
                                      : 'Guest User',
                              contactPersonNumber: enteredFullPhone,
                              address:
                                  checkoutController.guestAddress?.address ??
                                      prefAddress?.address ??
                                      '',
                              latitude:
                                  checkoutController.guestAddress?.latitude ??
                                      prefAddress?.latitude ??
                                      '0',
                              longitude:
                                  checkoutController.guestAddress?.longitude ??
                                      prefAddress?.longitude ??
                                      '0',
                              zoneId: checkoutController.guestAddress?.zoneId ??
                                  prefAddress?.zoneId,
                              email: guestEmailController.text.isNotEmpty
                                  ? guestEmailController.text
                                  : AuthHelper.getGuestEmail(),
                              streetNumber:
                            ...
                            String effectivePhone = hasProfilePhone
                                ? profilePhone
                                : enteredFullPhone;
                              ...
                              finalAddress = AddressModel(
                                ...
                                contactPersonName:
                                    '${Get.find<ProfileController>().userInfoModel?.fName ?? ''} ${Get.find<ProfileController>().userInfoModel?.lName ?? ''}'
                                        .trim(),
                                contactPersonNumber: effectivePhone,
                              );
                            } else {
                              if (!hasProfilePhone &&
                                  enteredPhoneText.isNotEmpty) {
                                finalAddress.contactPersonNumber =
                                    effectivePhone;
                              } else if (finalAddress.contactPersonNumber ==
                                      null ||
                          ...
                          String effectiveContactPhone = isGuestLogIn
                              ? enteredFullPhone
                              : (hasProfilePhone
                                  ? profilePhone
                                  : (enteredPhoneText.isNotEmpty
                          ...
                          if (widget.storeId == null ||
                            ...
                            for (int index = 0;
                              ...
                              carts.add(OnlineCart(
                                ...
                                price: cart.discountedPrice.toString(),
                                variant: '',
                                variation: Get.find<SplashController>()
                                        .getModuleConfig(cart.item!.moduleType)
                                        .newVariation!
                                    ? null
                                    : cart.variation,
                                variations: Get.find<SplashController>()
                                        .getModuleConfig(cart.item!.moduleType)
                                        .newVariation!
                                    ? variations
                                    : null,
                            ...
                            PlaceOrderBodyModel placeOrderBody =
                              ...
                              address: (checkoutController.orderType ==
                                          'pickup_center' &&
                                      checkoutController.selectedPickupCenter !=
                                          null)
                                  ? '${checkoutController.selectedPickupCenter!.name ?? ''} - ${checkoutController.selectedPickupCenter!.address ?? ''}'
                                  : finalAddress.address,
                              latitude: (checkoutController.orderType ==
                                          'pickup_center' &&
                                      checkoutController.selectedPickupCenter !=
                                          null)
                                  ? checkoutController
                                      .selectedPickupCenter!.latitude
                                  : finalAddress.latitude,
                              longitude: (checkoutController.orderType ==
                                          'pickup_center' &&
                                      checkoutController.selectedPickupCenter !=
                                          null)
                                  ? checkoutController
                                      .selectedPickupCenter!.longitude
                              ...
                              contactPersonNumber: effectiveContactPhone,
                              ...
                              isBuyNow: widget.fromCart ? 0 : 1,
                              guestEmail:
                                  isGuestLogIn ? finalAddress.email : null,
                              extraPackagingAmount:
                                  Get.find<CartController>().needExtraPackage
                                      ? checkoutController
                                          .store!.extraPackagingAmount
                              ...
                              productReferrerId: Get.find<AuthController>()
                                  ...
                                  : null,
                              saverDeliveryType:
                                  checkoutController.saverDeliveryType,
                              monthlySubscribe:
                                  checkoutController.monthlySubscribe,
                              pickupCenterName: checkoutController.orderType ==
                                      'pickup_center'
                                  ? checkoutController
                                      .selectedPickupCenter?.name
                                  : null,
                              phone: checkoutController.orderType ==
                                      'pickup_center'
                                  ? (checkoutController
  ...
  List<DropdownItem<int>> _getDropdownAddressList(
    ...
    List<AddressModel> addresses =
        _getAddressList(addressList: addressList, store: store);
  ...
  String _getAddressKey(AddressModel address) {
    String lat =
        (double.tryParse(address.latitude ?? '') ?? 0).toStringAsFixed(3);
    String lng =
        (double.tryParse(address.longitude ?? '') ?? 0).toStringAsFixed(3);
    ...
    if (addressList != null && addressList.isNotEmpty) {
      ...
      for (int index = 0; index < reversedList.length; index++) {
        ...
        if (store != null && store.zoneId != null) {
          ...
          bool matchZoneId = (item.zoneId != null &&
              item.zoneId.toString() == targetZone.toString());
          bool matchZoneIds = (item.zoneIds != null &&
              item.zoneIds!.any((z) => z.toString() == targetZone.toString()));
          inZone = matchZoneId || matchZoneIds || item.zoneId == null;
        }
        if (inZone) {
          if (!(item.latitude == '15.369445' &&
              item.longitude == '44.191006')) {
        ...
        if (zData.modules != null) {
          for (Modules m in zData.modules!) {
            if (m.id == currentModuleId &&
                m.pivot != null &&
                (m.pivot!.zoneId == store.zoneId || zData.id == store.zoneId)) {
              moduleData = m.pivot;
              break;
            }
          }
  ...
  double _calculatePrice(
    ...
    if (cartList != null) {
      for (var cartModel in cartList) {
        ...
        bool isNewVariation = (Get.find<SplashController>()
              ...
              if (index < cartModel.foodVariations!.length &&
                ...
                for (int i = 0;
                  ...
                  if (i < cartModel.foodVariations![index].length &&
                    ...
                    double optPrice = cartModel.item!.foodVariations![index]
                            .variationValues![i].optionPrice ??
                        0;
                    int qty = cartModel.quantity ?? 1;
                    double? converted = PriceConverter.convertWithDiscount(
                        optPrice, discount, discountType,
  ...
  double _calculateSingleOriginalDeliveryCharge(
      {required Store? store,
    ...
    Pivot? moduleData;
    List<ZoneData>? zList = address.zoneData ??
    ...
    if (store != null && store.selfDeliverySystem == 1) {
      perKmCharge = isBatchedOrder
          ? (store.perKmShippingChargeGroup ?? store.perKmShippingCharge ?? 0)
          : (store.perKmShippingCharge ?? 0);
      minimumCharge = isBatchedOrder
          ? (store.minimumShippingChargeGroup ??
      ...
      perKmCharge = isBatchedOrder
          ? (moduleData.perKmShippingChargeGroup ??
              store.perKmShippingChargeGroup ??
              moduleData.perKmShippingCharge ??
              0)
          : (moduleData.perKmShippingCharge ?? 0);
      minimumCharge = isBatchedOrder
          ? (moduleData.minimumShippingChargeGroup ??
              store.minimumShippingChargeGroup ??
              moduleData.minimumShippingCharge ??
              0)
          : (moduleData.minimumShippingCharge ?? 0);
      ...
      perKmCharge =
          (store.perKmShippingCharge != null && store.perKmShippingCharge! > 0)
              ? store.perKmShippingCharge!
              : (configModel?.perKmShippingCharge ?? 150);
      minimumCharge = (store.minimumShippingCharge != null &&
              store.minimumShippingCharge! > 0)
          ? store.minimumShippingCharge!
    ...
    if (checkoutController.orderType == 'pickup_center') {
      ...
      if (checkoutController.selectedPickupCenter != null &&
          checkoutController.selectedPickupCenter!.latitude != null) {
        double? sLat = double.tryParse(store?.latitude ?? '');
        double? sLng = double.tryParse(store?.longitude ?? '');
        double? pLat =
            double.tryParse(checkoutController.selectedPickupCenter!.latitude!);
        double? pLng = double.tryParse(
        ...
        if (sLat != null &&
            ...
            pLat != 0) {
          calcDistance =
              Geolocator.distanceBetween(sLat, sLng, pLat, pLng) / 1000;
        }
      }
      ...
      bool isSameZone = store != null &&
          checkoutController.selectedPickupZone != null &&
          store.zoneId == checkoutController.selectedPickupZone!.id;

      String? chargeType = isSameZone
          ? moduleData?.pickupCenterChargeType
          : moduleData?.pickupCenterOutsideChargeType;
      double fixedCharge = isSameZone
          ? (moduleData?.pickupCenterFixedCharge ?? 0)
          : (moduleData?.pickupCenterOutsideFixedCharge ?? 0);
      double pkCharge = isSameZone
          ? (moduleData?.pickupCenterPerKmCharge ?? 0)
          : (moduleData?.pickupCenterOutsidePerKmCharge ?? 0);
      double minCharge = isSameZone
          ? (moduleData?.pickupCenterMinimumCharge ?? 0)
          : (moduleData?.pickupCenterOutsideMinimumCharge ?? 0);
    ...
    if (store != null && store.latitude != null && address.latitude != null) {
      ...
      if (sLat != null &&
          ...
          sLat != 0) {
        calcDistance =
            Geolocator.distanceBetween(sLat, sLng, aLat, aLng) / 1000;
      }
    }
    if (calcDistance == 0 && distance != null && distance > 0) {
      calcDistance = distance;
  ...
  double _calculateOriginalDeliveryCharge(
    ...
    if (checkoutController.serverOriginalDeliveryCharge != null) {
      return checkoutController.serverOriginalDeliveryCharge!;
    }
    if (checkoutController.isLoadingDeliveryFee) {
      return -1;
    }

    List<Store>? stores = checkoutController.stores;
    if (stores != null &&
        stores.isNotEmpty &&
        checkoutController.storeDistances.isNotEmpty) {
      double totalBaseCharge = 0;
      for (Store s in stores) {
        double dist = checkoutController.storeDistances[s.id] ?? 0;
        totalBaseCharge += _calculateSingleOriginalDeliveryCharge(
            store: s,
            address: address,
            distance: dist,
            extraCharge: 0,
    ...
    return _calculateSingleOriginalDeliveryCharge(
        store: store,
        address: address,
        distance: distance,
...
*/
  ...
  double _calculateDeliveryCharge(
    ...
    final checkoutController = Get.find<CheckoutController>();
    if (checkoutController.serverDeliveryCharge != null) {
      return checkoutController.serverDeliveryCharge!;
    }
    if (checkoutController.isLoadingDeliveryFee) {
      return -1;
    }
    ...
    if (checkoutController.serverDeliveryCharge == null &&
        !checkoutController.isLoadingDeliveryFee &&
    ...
    if (orderType != 'take_away' &&
        ...
        Get.find<CheckoutController>().storeDistances.isEmpty) {
      return -1;
    }
    ...
    if (stores != null && stores.isNotEmpty) {
      if (orderType != 'take_away' &&
          Get.find<CheckoutController>().storeDistances.length <
              stores.length) {
        return -1;
      }
      ...
      for (Store s in stores) {
        ...
        double baseFee = _calculateSingleOriginalDeliveryCharge(
            store: s,
            address: address,
            distance: dist,
            extraCharge: 0,
        ...
        bool isAdminFreeAll = configModel?.adminFreeDelivery?.status == true &&
            configModel?.adminFreeDelivery?.type ==
                'free_delivery_to_all_store';
        bool isAdminFreeOver = configModel?.adminFreeDelivery?.status == true &&
            configModel?.adminFreeDelivery?.type ==
      ...
      if (anyStoreRequiresDelivery) {
        ...
        if (store != null &&
            ...
            surgePrice > 0) {
          if (surgePriceType == 'percent') {
            totalBaseDeliveryCharge +=
                (totalBaseDeliveryCharge * (surgePrice / 100));
          } else {
    ...
    double deliveryCharge = _calculateSingleOriginalDeliveryCharge(
        store: store,
        address: address,
        distance: distance,
        extraCharge: extraCharge,
  ...
  bool _checkZoneOfflinePaymentOnOff(
    ...
    bool status = false;
    ZoneData? zoneData;
    List<ZoneData>? zList = addressModel?.zoneData ??
        AddressHelper.getUserAddressFromSharedPref()?.zoneData;
    if (zList != null) {
    ...
    status = zoneData?.offlinePayment ??
  ...
  bool _checkPrescriptionRequired() {
    bool attachmentReq = Get.find<SplashController>()
            .configModel
            ?.moduleConfig
            ?.module
            ?.orderAttachment ??
  ...
  double _calculateProDiscount({
    required List<CartModel?>? cartList,
    required ProActiveBenefit? benefit,
    required double fallbackSubTotal,
    required double fallbackDiscount,
    required double fallbackCouponDiscount,
  }) {
    if (benefit == null || benefit.type != ProBenefitType.discount) return 0;

    if (cartList != null && cartList.isNotEmpty) {
      Map<int, List<CartModel>> storeCartMap = {};
      for (var cart in cartList) {
        if (cart?.item != null) {
          int sId = cart!.item!.storeId ?? 0;
          storeCartMap.putIfAbsent(sId, () => []).add(cart);
        }
      }

      if (storeCartMap.isNotEmpty) {
        double totalProDiscount = 0;
        final checkoutController = Get.find<CheckoutController>();
        final List<Store>? allStores = checkoutController.stores;

        for (var entry in storeCartMap.entries) {
          int storeId = entry.key;
          List<CartModel> storeCarts = entry.value;

          Store? matchedStore;
          if (allStores != null) {
            matchedStore = allStores.firstWhereOrNull((s) => s.id == storeId);
          }
          matchedStore ??= checkoutController.store;

          double storePrice =
              _calculatePrice(store: matchedStore, cartList: storeCarts);
          double storeAddOns =
              _calculateAddonsPrice(store: matchedStore, cartList: storeCarts);
          double storeVariations = _calculateVariationPrice(
              store: matchedStore,
              cartList: storeCarts,
              calculateWithoutDiscount: true);
          double storeSubTotal = _calculateSubTotal(
              price: storePrice,
              addOns: storeAddOns,
          ...
          final bool meetsMinOrder = benefit.minOrderStatus != true ||
              storeSubTotal >= (benefit.minOrderAmount ?? 0);
          if (meetsMinOrder) {
            final double base = (storeSubTotal - storeDiscount)
                .clamp(0, double.infinity)
                .toDouble();
            double storeProDisc = base * ((benefit.percentage ?? 0) / 100);
            if (benefit.maxAmount != null &&
                benefit.maxAmount! > 0 &&
                storeProDisc > benefit.maxAmount!) {
              storeProDisc = benefit.maxAmount!;
            }
            if (storeProDisc > base) storeProDisc = base;
            totalProDiscount += storeProDisc;
          }
        }
        return PriceConverter.toFixed(totalProDiscount);
      }
    }

    final bool meetsMinOrder = benefit.minOrderStatus != true ||
        fallbackSubTotal >= (benefit.minOrderAmount ?? 0);
    if (!meetsMinOrder) return 0;
    final double base =
        fallbackSubTotal - fallbackDiscount - fallbackCouponDiscount;
    if (base < 0) return 0;
  ...
  double _calculateProDeliveryDiscount(
      double base, double deliveryCharge, ProActiveBenefit? benefit) {
    if (benefit == null || benefit.type != ProBenefitType.deliveryFee) return 0;
    if (deliveryCharge <= 0) return 0;
    final bool meetsMinOrder =
        benefit.minOrderStatus != true || base >= (benefit.minOrderAmount ?? 0);
    if (!meetsMinOrder) return 0;
    if (benefit.offerType == ProOfferType.fullFree)
      return PriceConverter.toFixed(deliveryCharge);
    double discount =
        deliveryCharge * ((benefit.chargeDiscountPercentage ?? 0) / 100);
    return PriceConverter.toFixed(discount);
  }
