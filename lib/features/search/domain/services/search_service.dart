...
class SearchService implements SearchServiceInterface {
  ...
  @override
  Future<Response> getSearchByAiData(String? query) async {
    return await searchRepositoryInterface.getSearchByAiData(query);
  }
  ...
  List<Item>? sortItemSearchList( List<Item>? allItemList, double upperValue, double lowerValue, int rating, bool veg, bool nonVeg, bool isAvailableItems, bool isDiscountedItems, int sortIndex) {
    ...
    if(sortIndex != -1) {
      if(sortIndex == 0) {
        ...
        searchItemList.sort((a, b) => a.price!.compareTo(b.price!));
      } else if(sortIndex == 3) {
        searchItemList.sort((a, b) => b.price!.compareTo(a.price!));
      } else if(sortIndex == 4) {
        searchItemList.sort((a, b) => b.avgRating!.compareTo(a.avgRating!));
      } else if(sortIndex == 5) {
        searchItemList.sort((a, b) => b.discount!.compareTo(a.discount!));
      } else if(sortIndex == 6) {
        searchItemList.sort((a, b) => b.id!.compareTo(a.id!)); // Assuming ID corresponds to date/newness
      }
    }
  ...
  List<Store>? sortStoreSearchList(List<Store>? allStoreList, int storeRating, bool storeVeg, bool storeNonVeg, bool isAvailableStore, bool isDiscountedStore, int storeSortIndex) {
    ...
    if(storeSortIndex != -1) {
      ...
      } else if(storeSortIndex == 1) {
        searchStoreList.sort((a, b) => b.name!.toLowerCase().compareTo(a.name!.toLowerCase()));
      } else if(storeSortIndex == 2) {
        // Price sort for stores? Maybe based on minimum order?
        searchStoreList.sort((a, b) => a.minimumOrder!.compareTo(b.minimumOrder!));
      } else if(storeSortIndex == 3) {
        searchStoreList.sort((a, b) => b.minimumOrder!.compareTo(a.minimumOrder!));
      } else if(storeSortIndex == 4) {
        searchStoreList.sort((a, b) => b.avgRating!.compareTo(a.avgRating!));
      } else if(storeSortIndex == 5) {
        // Discount sort for stores
        searchStoreList.sort((a, b) => (b.discount != null ? 1 : 0).compareTo(a.discount != null ? 1 : 0));
      }
    }
