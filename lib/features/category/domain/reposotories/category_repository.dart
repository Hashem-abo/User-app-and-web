...
class CategoryRepository implements CategoryRepositoryInterface {
  ...
  Future<Response> getSearchData(String? query, String? categoryID, bool isStore, String type) async {
    return await apiClient.getData(
      '${AppConstants.searchUri}${isStore ? 'stores' : 'items'}/search?name=$query&category_id=$categoryID&type=$type&offset=1&limit=12',
    );
  }
  ...
  Future<CategoryModel?> getCategoryDetails(String id) async {
    ...
    Response response = await apiClient.getData('${AppConstants.categoryDetailsUri}$id');
    if (response.statusCode == 200) {
      category = CategoryModel.fromJson(response.body);
    }
    return category;
  }
