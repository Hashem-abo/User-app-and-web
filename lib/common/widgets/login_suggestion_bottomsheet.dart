...
import 'package:suliman/features/auth/widgets/sign_in/existing_user_bottom_sheet.dart';
            ...
            child: Column(
              ...
              children: [
                ...
                if(facebookLoginActive || googleLoginActive || appleLoginActive) ...[
                  ...
                  Row(
                    children: [
                      const Expanded(child: Divider(color: Color(0xFFE4E7EC), height: 1, thickness: 1)),
                      Padding(
                  ...
                  Column(
                    ...
                    children: [
                      if(googleLoginActive) ...[
                        ...
                        if(facebookLoginActive || appleLoginActive) const SizedBox(height: Dimensions.paddingSizeDefault),
                      ],
                      ...
                      if(facebookLoginActive) ...[
                        ...
                        if(appleLoginActive) const SizedBox(height: Dimensions.paddingSizeDefault),
                      ],
    ...
    if(response.isSuccess && response.authResponseModel != null && response.authResponseModel!.isExistUser != null) {
      ...
      if (AuthHelper.isLoggedIn() && Get.isRegistered<ProfileController>()) {
        ...
        if (!hasPhone || !hasGender) {
          ...
          if (ResponsiveHelper.isDesktop(Get.context)) {
            Get.back();
            Get.dialog(NewUserSetupScreen(name: displayName ?? '', loginType: CentralizeLoginType.social.name, phone: hasPhone ? phone : '', email: email, backFromThis: true));
          } else {
            Get.toNamed(RouteHelper.getNewUserSetupScreen(name: displayName ?? '', loginType: CentralizeLoginType.social.name, phone: hasPhone ? phone : '', email: email, backFromThis: true));
          }
          return;
        }
      }
